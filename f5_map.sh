...
## Path to all the F5 root keys.
# The name of the key must be the name of the host.
pemPath="/srv/ssh_keys/f5"
...
# User must be part of "ssh_keys" group for this script to work.
[[ "$(getent group ssh_keys)" == *"$(whoami)"* ]] || {
    echo >&2 "$(basename "${0}"): You are *not* a member of the \"ssh_keys\" group. Aborting."
    exit 1
}
...
[[ ! "${RUN:-n}" == "y" ]] && {
    ...
    for id in ${pemPath}/*.pem; do
        ...
        if [[ ! -f "${cache}" ]] &&  [[ `find "${cache}" -mmin +60 2>&1` ]]; then
            install -m 644 -D <(ssh -o "StrictHostKeyChecking no" -i "${tmpID}" root@${f5host} 'bash -c "$(base64 -d <<< '''${cmd}''')"' 2> /dev/null) "${cache}"
        fi
