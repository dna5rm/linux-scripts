...
function openai_chat() {
    ...
    jq -e '.' >/dev/null 2>&1 <<<${chat_data} > /dev/null && {
        ...
        [[ ! -z "${user_input[@]}" ]] && {
            ...
            chat_data=`jq -c --arg input "$(echo -e "${user_input[@]}")" '.messages += [{"role": "user", "content": $input}]' <<<${chat_data}`
            unset user_input
        } || { exit 0; }

        # Fetch the response & cache, ignore cache if older than 30 days.
        if [[ ! -f "${CACHE}/${HASH}" ]] || [[ $(find "${CACHE}/${HASH}" -mtime +30 -print) ]]; then
            ...
            case "${http_code}" in
                400)
                    # Return the response
                    echo "$(basename "${FUNC_SOURCE}"): $(jq -r '.error | "\(.type) - \(.message)"' <(sed '$ d' <<<${response}))"
                    exit 1;;
                *)
                    # Return the content (fresh).
                    chat_data=`jq -c --arg content "$(jq -r '.choices[0]|.message.content' <(sed '$ d' <<<${response}))" '.messages += [{"role": "assistant", "content": $content}]' <<<${chat_data}`
                    ;;
            esac
            ...
            response="$(printf "$(sed -n '/{\"role\":\"assistant\",\"content\":\"/,/\"}],\"/ {s/.*"content":\"//;s/\"}].*,\".*//;p}' "${CACHE}/${HASH}")")"

            # Return the content (cached).
            chat_data=`jq -c --arg content "${response}" '.messages += [{"role": "assistant", "content": $content}]' <<<${chat_data}`
        fi
