...
function postDeletinganalertrule ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Deleting an alert rule
	Ref: /alert-rules/{ruleId}/delete.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${thousandeyes_uri}/alert-rules/${1}/delete.json" \
