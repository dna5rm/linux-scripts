...
function getEndpointusersessionlist ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Endpoint user session list
	Ref: /endpoint-data/user-sessions.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request GET --url "${thousandeyes_uri}/endpoint-data/user-sessions.json" \
