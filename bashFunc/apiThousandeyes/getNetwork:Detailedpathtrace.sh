## Network:Detailedpathtrace # Network: Detailed path trace
# /net/path-vis/{testId}/{agentId}/{roundId}.json

function Network:Detailedpathtrace ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Network: Detailed path trace
	Ref: /net/path-vis/{testId}/{agentId}/{roundId}.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request GET --url "${thousandeyes_uri}/net/path-vis/${1}/${2}/${3}" \
