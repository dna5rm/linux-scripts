...
function postUpdatinganaccountgroup ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Updating an account group
	Ref: /account-groups/{aid}/update.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${thousandeyes_uri}/account-groups/${1}/update.json" \
