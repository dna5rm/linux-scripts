...
function Updatinganalertsuppressionwindow ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Updating an alert suppression window
	Ref: /alert-suppression-windows/{alertSuppressionWindowId}/update.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${thousandeyes_uri}/alert-suppression-windows/${1}/update.json" \
