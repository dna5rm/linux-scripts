...
function getNetwork:BGProuteinformation ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Network: BGP route information
	Ref: /net/bgp-routes/{testId}/{prefixId}/{roundId}.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request GET --url "${thousandeyes_uri}/net/bgp-routes/${1}/${2}/${3}" \
