...
###################
[[ ! -f "_swagger.json" ]] && {
    jq -r '.payload.blob.rawLines[]' <(curl --silent https://github.com/ciscomanagedservices/sxo-thousandeyes/blob/main/Cisco%20ThousandEyes%20API%20v6.json) > _swagger.json
}
###################
...
######### Generate API function script.
	...
	function ${method,,}${operationId} ()
	{
	    ...
	    if [[ -z "\${thousandeyes_uri}" ]] || [[ -z "\${auth_key}" ]]` [[ ${inputs:-0} != "0" ]] && { echo " || [[ -z \"\\\${${inputs}}\" ]]"; }`; then
	$(printf "\t")cat <<-EOF
	$(printf "\t")\$(basename "\${0}" 2> /dev/null):\${FUNCNAME[0]} - ${summary}
	$(printf "\t")Ref: ${path:-null}
	$(printf "\t")---
	$(printf "\t")API Base URI: \\\${thousandeyes_uri} (\${thousandeyes_uri:-required})
	...
	$(printf "\t")EOF
	        ...
	        curl --silent --insecure --location --request ${method^^} --url "\${thousandeyes_uri}/$(for i in $(echo ${path} | sed 's/\// /g'); do [[ "${i:0:1}" != "{" ]] && { echo "${i}"; } || { echo "\${$((${pcount:-0}+1))}"; let ++pcount; }; done | sed -z 's/\n/\//g;s/\/$//')" \\
