...
function getDNS:Servermetrics ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - DNS: Server metrics
	Ref: /dns/server/{testId}/{serverId}.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request GET --url "${thousandeyes_uri}/dns/server/${1}/${2}" \
