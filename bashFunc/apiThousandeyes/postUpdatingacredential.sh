...
function postUpdatingacredential ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Updating a credential
	Ref: /credentials/{credentialId}/update.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${thousandeyes_uri}/credentials/${1}/update.json" \
