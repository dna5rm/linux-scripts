...
function getEndpointscheduledtestlistbytype ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Endpoint scheduled test list by type
	Ref: /endpoint-tests/{testType}.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request GET --url "${thousandeyes_uri}/endpoint-tests/${1}" \
