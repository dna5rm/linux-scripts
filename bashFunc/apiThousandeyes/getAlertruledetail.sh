...
function getAlertruledetail ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Alert rule detail
	Ref: /alert-rules/{ruleId}.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request GET --url "${thousandeyes_uri}/alert-rules/${1}" \
