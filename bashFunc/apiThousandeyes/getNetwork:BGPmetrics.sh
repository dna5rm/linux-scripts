...
function getNetwork:BGPmetrics ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Network: BGP metrics
	Ref: /net/bgp-metrics/{testId}.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request GET --url "${thousandeyes_uri}/net/bgp-metrics/${1}" \
