...
function DNS+:Mappings ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - DNS+: Mappings
	Ref: /dnsp/mappings/{testId}.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request GET --url "${thousandeyes_uri}/dnsp/mappings/${1}" \
