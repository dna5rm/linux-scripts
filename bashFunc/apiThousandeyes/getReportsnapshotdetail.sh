...
function Reportsnapshotdetail ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Report snapshot detail
	Ref: /report-snapshots/{snapshotId}.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request GET --url "${thousandeyes_uri}/report-snapshots/${1}" \
