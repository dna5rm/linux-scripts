...
function getEndpointnetworksessionslist ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Endpoint network sessions list
	Ref: /endpoint-data/user-sessions/network.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request GET --url "${thousandeyes_uri}/endpoint-data/user-sessions/network.json" \
