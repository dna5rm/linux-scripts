...
function DNS+:Resolutiontime ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - DNS+: Resolution time
	Ref: /dnsp/resolution-time/{testId}.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request GET --url "${thousandeyes_uri}/dnsp/resolution-time/${1}" \
