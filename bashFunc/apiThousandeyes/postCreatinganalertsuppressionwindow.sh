...
function postCreatinganalertsuppressionwindow ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Creating an alert suppression window
	Ref: /alert-suppression-windows/new.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${thousandeyes_uri}/alert-suppression-windows/new.json" \
