...
function postDeletingacredential ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Deleting a credential
	Ref: /credentials/{credentialId}/delete.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${thousandeyes_uri}/credentials/${1}/delete.json" \
