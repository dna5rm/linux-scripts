...
function getGetWeb:HTTPserver ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Web: HTTP server
	Ref: /endpoint-data/tests/web/http-server/{testId}.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request GET --url "${thousandeyes_uri}/endpoint-data/tests/web/http-server/${1}" \
