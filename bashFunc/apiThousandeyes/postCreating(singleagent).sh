...
function postCreating(singleagent) ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Creating (single agent)
	Ref: /agents/{agentId}/add-to-cluster.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${thousandeyes_uri}/agents/${1}/add-to-cluster.json" \
