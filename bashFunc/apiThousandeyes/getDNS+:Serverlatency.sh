...
function getDNS+:Serverlatency ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - DNS+: Server latency
	Ref: /dnsp/server-latency/{testId}.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request GET --url "${thousandeyes_uri}/dnsp/server-latency/${1}" \
