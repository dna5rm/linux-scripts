## Web:Transactiondetail # Web: Transaction detail
# /web/transactions/{testId}/{agentId}/{roundId}.json

function getWeb:Transactiondetail ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Web: Transaction detail
	Ref: /web/transactions/{testId}/{agentId}/{roundId}.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request GET --url "${thousandeyes_uri}/web/transactions/${1}/${2}/${3}" \
