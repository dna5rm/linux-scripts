...
function Alertsuppressionwindows ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Alert suppression windows
	Ref: /alert-suppression-windows.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request GET --url "${thousandeyes_uri}/alert-suppression-windows.json" \
