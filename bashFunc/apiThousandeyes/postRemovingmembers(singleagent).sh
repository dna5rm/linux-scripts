...
function postRemovingmembers(singleagent) ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Removing members (single agent)
	Ref: /agents/{agentId}/remove-from-cluster.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${thousandeyes_uri}/agents/${1}/remove-from-cluster.json" \
