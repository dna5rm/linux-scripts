...
function Voice:SIPserver ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Voice: SIP server
	Ref: /voice/sip-server/{testId}.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request GET --url "${thousandeyes_uri}/voice/sip-server/${1}" \
