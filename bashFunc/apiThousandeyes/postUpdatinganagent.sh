## Updatinganagent # Updating an agent
# /endpoint-agents/{agentId}.json

function Updatinganagent ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Updating an agent
	Ref: /endpoint-agents/{agentId}.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${thousandeyes_uri}/endpoint-agents/${1}" \
