...
function getWeb:WebTransactioncomponentdetail ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Web: Web Transaction component detail
	Ref: /web/web-transactions/{testId}/{agentId}/{roundId}/{pageNum}.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request GET --url "${thousandeyes_uri}/web/web-transactions/${1}/${2}/${3}/${4}" \
