...
function postEnablinganagent ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Enabling an agent
	Ref: /endpoint-agents/{agentId}/enable.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${thousandeyes_uri}/endpoint-agents/${1}/enable.json" \
