...
function Deletingalabel ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Deleting a label
	Ref: /groups/{groupId}/delete.json
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${thousandeyes_uri}/groups/${1}/delete.json" \
