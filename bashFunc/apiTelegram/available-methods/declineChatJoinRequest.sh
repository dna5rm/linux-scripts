...
function declineChatJoinRequest ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Use this method to decline a chat join request.
  ...
  user_id     Integer             Yes        Unique identifier of the target user
	EOF
        ...
        curl --silent --location \
          --request POST --url "https://api.telegram.org/bot${TELEGRAM_TOKEN}/declineChatJoinRequest" \
