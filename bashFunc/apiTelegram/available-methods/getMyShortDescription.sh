...
function getMyShortDescription ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Use this method to get the current bot short description for the given user language.
...
user language. Returns BotShortDescription on success.
  ...
  language_code   String   Optional   A two-letter ISO 639-1 language code or an empty string
	EOF
                ...
                cmd+=( "--url \"https://api.telegram.org/bot${TELEGRAM_TOKEN}/getMyShortDescription\"" )
