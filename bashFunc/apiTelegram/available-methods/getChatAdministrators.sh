...
function getChatAdministrators ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Use this method to get a list of administrators in a chat, which aren't bots.
...
bots. Returns an Array of ChatMember objects.
  ...
  chat_id     Integer or String   Yes        Unique identifier for the target chat or username of the target supergroup or channel (in the format `@channelusername`)
	EOF
                ...
                cmd+=( "--url \"https://api.telegram.org/bot${TELEGRAM_TOKEN}/getChatAdministrators\"" )
