...
function unbanChatSenderChat ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Use this method to unban a previously banned channel chat in a supergroup or channel.
...
success.
  ...
  sender_chat_id   Integer             Yes        Unique identifier of the target sender chat
	EOF
                ...
                cmd+=( "--url \"https://api.telegram.org/bot${TELEGRAM_TOKEN}/unbanChatSenderChat\"" )
