...
function EncryptedPassportElement ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Describes documents or other Telegram Passport elements shared with the bot by the user.
  ...
  hash           String                  Base64-encoded element hash for using in PassportElementErrorUnspecified
	EOF
        ...
        curl --silent --location \
          --request POST --url "https://api.telegram.org/bot${TELEGRAM_TOKEN}/EncryptedPassportElement" \
