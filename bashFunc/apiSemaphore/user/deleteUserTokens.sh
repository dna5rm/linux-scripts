## deleteUserTokens # Expires API token
...
function deleteUserTokens ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Expires API token
	Ref: /user/tokens/{api_token_id}
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${semaphore_api}/user/tokens/${1}" \
