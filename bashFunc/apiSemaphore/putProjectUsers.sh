## putProjectUsers # Update user role
# /project/{project_id}/users/{user_id}

function putProjectUsers ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update user role
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request PUT --url "${semaphore_api}/project/${1}/users/${2}" \
