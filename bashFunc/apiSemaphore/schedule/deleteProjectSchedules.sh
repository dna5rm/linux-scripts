## deleteProjectSchedules # Deletes schedule
# /project/{project_id}/schedules/{schedule_id}

function deleteProjectSchedules ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Deletes schedule
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${semaphore_api}/project/${1}/schedules/${2}" \
