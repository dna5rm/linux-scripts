## getAuthOidcRedirect # Finish OIDC authentication flow, upon succes you will be logged in
# /auth/oidc/{provider_id}/redirect

function getAuthOidcRedirect ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Finish OIDC authentication flow, upon succes you will be logged in
	Ref: /auth/oidc/{provider_id}/redirect
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request GET --url "${semaphore_api}/auth/oidc/${1}/redirect" \
        ...
        case "${http_code}" in
            302)
                echo "[302] Redirection to the Semaphore root URL on success, or to the login page on error"
                ;;
            *)
                # return the content
                sed '$ d' <<< "${response}"
