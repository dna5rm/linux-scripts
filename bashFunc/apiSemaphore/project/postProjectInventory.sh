## postProjectInventory # create inventory
# /project/{project_id}/inventory

function postProjectInventory ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - create inventory
	Ref: /project/{project_id}/inventory
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${semaphore_api}/project/${1}/inventory" \
