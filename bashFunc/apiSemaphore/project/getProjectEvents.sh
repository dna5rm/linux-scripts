## getProjectEvents # Get Events related to this project
...
function getProjectEvents ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get Events related to this project
	Ref: /project/{project_id}/events
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request GET --url "${semaphore_api}/project/${1}/events" \
