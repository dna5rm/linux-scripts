## postProjectTasks # Starts a job
# /project/{project_id}/tasks

function postProjectTasks ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Starts a job
	Ref: /project/{project_id}/tasks
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${semaphore_api}/project/${1}/tasks" \
