## deleteProjectInventory # Removes inventory
# /project/{project_id}/inventory/{inventory_id}

function deleteProjectInventory ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Removes inventory
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${semaphore_api}/project/${1}/inventory/${2}" \
