## postProjectViews # create view
# /project/{project_id}/views

function postProjectViews ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - create view
	Ref: /project/{project_id}/views
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${semaphore_api}/project/${1}/views" \
