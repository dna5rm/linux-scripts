## deleteProjectTemplates # Removes template
# /project/{project_id}/templates/{template_id}

function deleteProjectTemplates ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Removes template
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${semaphore_api}/project/${1}/templates/${2}" \
