## getProjectTasksLast # Get last 200 Tasks related to current project
# /project/{project_id}/tasks/last

function getProjectTasksLast ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get last 200 Tasks related to current project
	Ref: /project/{project_id}/tasks/last
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request GET --url "${semaphore_api}/project/${1}/tasks/last" \
