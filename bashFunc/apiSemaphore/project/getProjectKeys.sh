## getProjectKeys # Get access keys linked to project
...
function getProjectKeys ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get access keys linked to project
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request GET --url "${semaphore_api}/project/${1}/keys" \
