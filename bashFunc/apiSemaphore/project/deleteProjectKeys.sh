## deleteProjectKeys # Removes access key
# /project/{project_id}/keys/{key_id}

function deleteProjectKeys ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Removes access key
	Ref: /project/{project_id}/keys/{key_id}
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${semaphore_api}/project/${1}/keys/${2}" \
