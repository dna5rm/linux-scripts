## putProjectKeys # Updates access key
# /project/{project_id}/keys/{key_id}

function putProjectKeys ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Updates access key
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request PUT --url "${semaphore_api}/project/${1}/keys/${2}" \
