## getProjectTasksOutput # Get task output
# /project/{project_id}/tasks/{task_id}/output

function getProjectTasksOutput ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get task output
	Ref: /project/{project_id}/tasks/{task_id}/output
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request GET --url "${semaphore_api}/project/${1}/tasks/${2}/output" \
