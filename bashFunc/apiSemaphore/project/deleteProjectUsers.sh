## deleteProjectUsers # Removes user from project
...
function deleteProjectUsers ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Removes user from project
	Ref: /project/{project_id}/users/{user_id}
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${semaphore_api}/project/${1}/users/${2}" \
