## deleteProjectRepositories # Removes repository
# /project/{project_id}/repositories/{repository_id}

function deleteProjectRepositories ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Removes repository
	Ref: /project/{project_id}/repositories/{repository_id}
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${semaphore_api}/project/${1}/repositories/${2}" \
