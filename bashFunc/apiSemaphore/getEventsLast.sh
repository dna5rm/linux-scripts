...
function getEventsLast ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get last 200 Events related to Semaphore and projects you are part of
	Ref: /events/last
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request GET --url "${semaphore_api}/events/last" \
