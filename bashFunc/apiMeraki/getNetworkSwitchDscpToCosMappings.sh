...
#
# Ref: https://developer.cisco.com/meraki/api-latest/#!get-network-switch-dscp-to-cos-mappings
...
function getNetworkSwitchDscpToCosMappings ()
{
	...
	curl --silent --location --request GET --url "${meraki_uri}/networks/${1}/switch/dscpToCosMappings" --header "Content-Type: application/json" --header "Accept: application/json" --header "X-Cisco-Meraki-API-Key: ${auth_key}"
    fi
}
