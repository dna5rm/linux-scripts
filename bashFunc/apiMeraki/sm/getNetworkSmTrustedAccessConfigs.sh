...
function getNetworkSmTrustedAccessConfigs ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - List Trusted Access Configs
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-sm-trusted-access-configs
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/sm/trustedAccessConfigs" \
