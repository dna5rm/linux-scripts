...
function getNetworkSmDeviceDesktopLogs ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return historical records of various Systems Manager network connection details for desktop devices.
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-sm-device-desktop-logs
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/sm/devices/${2}/desktopLogs" \
