...
function getNetworkSmDeviceConnectivity ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Returns historical connectivity data (whether a device is regularly checking in to Dashboard).
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-sm-device-connectivity
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/sm/devices/${2}/connectivity" \
