...
function getNetworkSmUserAccessDevices ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - List User Access Devices and its Trusted Access Connections
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-sm-user-access-devices
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/sm/userAccessDevices" \
