...
#
# Ref: https://developer.cisco.com/meraki/api-v1/#!get-device-switch-ports
...
function getDeviceSwitchPorts ()
{
	...
	curl --silent --location --request GET --url "${meraki_uri}/devices/${1}/switch/ports" --header "Content-Type: application/json" --header "Accept: application/json"  --header "X-Cisco-Meraki-API-Key: ${auth_key}";
    fi
}
