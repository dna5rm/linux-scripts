...
#
# Ref: https://developer.cisco.com/meraki/api-latest/#!get-network-wireless-settings
...
function getNetworkWirelessSettings ()
{
	...
	curl --silent --location --request GET --url "${meraki_uri}/networks/${1}/wireless/settings" --header "Content-Type: application/json" --header "Accept: application/json" --header "X-Cisco-Meraki-API-Key: ${auth_key}"
    fi
}
