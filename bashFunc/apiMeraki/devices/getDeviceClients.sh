...
function getDeviceClients ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - List the clients of a device, up to a maximum of a month ago
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-device-clients
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/devices/${1}/clients" \
