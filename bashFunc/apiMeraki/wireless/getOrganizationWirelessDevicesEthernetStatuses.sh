...
function getOrganizationWirelessDevicesEthernetStatuses ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Endpoint to see power status for wireless devices
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-organization-wireless-devices-ethernet-statuses
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/organizations/${1}/wireless/devices/ethernet/statuses" \
