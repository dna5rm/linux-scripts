...
function getNetworkWirelessLatencyStats ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Aggregated latency info for this network
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-wireless-latency-stats
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/wireless/latencyStats" \
