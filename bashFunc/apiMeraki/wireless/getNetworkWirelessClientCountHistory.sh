...
function getNetworkWirelessClientCountHistory ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return wireless client counts over time for a network, device, or network client
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-wireless-client-count-history
	---
	...
	clientId: Filter results by network client to return per-device client counts over time inner joined by the queried client's connection history.
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/wireless/clientCountHistory" \
