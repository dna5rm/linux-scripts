...
function getNetworkWirelessUsageHistory ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return AP usage over time for a device or network client
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-wireless-usage-history
	---
	...
	clientId: Filter results by network client to return per-device AP usage over time inner joined by the queried client's connection history.
	deviceSerial: Filter results by device. Requires :band.
	apTag: Filter results by AP tag; either :clientId or :deviceSerial must be jointly specified.
	band: Filter results by band (either '2.4', '5' or '6').
	ssid: Filter results by SSID number.
	EOF
    else
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/wireless/usageHistory" \
