...
function getNetworkWirelessLatencyHistory ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return average wireless latency over time for a network, device, or network client
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-wireless-latency-history
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/wireless/latencyHistory" \
