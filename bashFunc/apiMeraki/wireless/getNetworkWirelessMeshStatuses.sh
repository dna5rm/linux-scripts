...
function getNetworkWirelessMeshStatuses ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - List wireless mesh statuses for repeaters
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-wireless-mesh-statuses
	---
	...
	perPage: The number of entries per page returned. Acceptable range is 3 - 500. Default is 50.
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/wireless/meshStatuses" \
