...
function getNetworkWirelessSignalQualityHistory ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return signal quality (SNR/RSSI) over time for a device or network client
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-wireless-signal-quality-history
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/wireless/signalQualityHistory" \
