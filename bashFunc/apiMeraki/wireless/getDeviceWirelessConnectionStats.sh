...
function getDeviceWirelessConnectionStats ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Aggregated connectivity info for a given AP on this network
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-device-wireless-connection-stats
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/devices/${1}/wireless/connectionStats" \
