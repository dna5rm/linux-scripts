...
function getNetworkWirelessClientLatencyStats ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Aggregated latency info for a given client on this network
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-wireless-client-latency-stats
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/wireless/clients/${2}/latencyStats" \
