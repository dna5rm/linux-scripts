...
function getNetworkWirelessDevicesConnectionStats ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Aggregated connectivity info for this network, grouped by node
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-wireless-devices-connection-stats
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/wireless/devices/connectionStats" \
