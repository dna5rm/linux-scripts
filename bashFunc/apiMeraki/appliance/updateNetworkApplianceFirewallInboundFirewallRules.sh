...
function updateNetworkApplianceFirewallInboundFirewallRules ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update the inbound firewall rules of an MX network
	Ref: https://developer.cisco.com/meraki/api-v1/#!update-network-appliance-firewall-inbound-firewall-rules
	---
	...
	EOF
        ...
        echo curl --silent --output /dev/null --write-out "%{http_code}\n" --location \
          --request PUT --url "${meraki_uri}/networks/${1}/appliance/firewall/inboundFirewallRules" \
