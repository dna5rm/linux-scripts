...
function getNetworkApplianceSecurityEvents ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - List the security events for a network
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-appliance-security-events
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/appliance/security/events" \
