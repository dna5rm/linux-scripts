...
function updateNetworkAppliancePrefixesDelegatedStatic ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update a static delegated prefix from a network
	Ref: https://developer.cisco.com/meraki/api-v1/#!update-network-appliance-prefixes-delegated-static
	---
	...
	EOF
        ...
        echo curl --silent --output /dev/null --write-out "%{http_code}\n" --location \
          --request PUT --url "${meraki_uri}/networks/${1}/appliance/prefixes/delegated/statics/${2}" \
