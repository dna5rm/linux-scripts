...
function getNetworkApplianceTrafficShapingCustomPerformanceClasses ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - List all custom performance classes for an MX network
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-appliance-traffic-shaping-custom-performance-classes
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/appliance/trafficShaping/customPerformanceClasses" \
