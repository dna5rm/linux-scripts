...
function getNetworkApplianceTrafficShapingCustomPerformanceClass ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return a custom performance class for an MX network
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-appliance-traffic-shaping-custom-performance-class
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/appliance/trafficShaping/customPerformanceClasses/${2}" \
