...
function deleteNetworkApplianceStaticRoute ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete a static route from an MX or teleworker network
	Ref: https://developer.cisco.com/meraki/api-v1/#!delete-network-appliance-static-route
	---
	...
	EOF
        ...
        echo curl --silent --output /dev/null --write-out "%{http_code}\n" --location \
          --request DELETE --url "${meraki_uri}/networks/${1}/appliance/staticRoutes/${2}" \
