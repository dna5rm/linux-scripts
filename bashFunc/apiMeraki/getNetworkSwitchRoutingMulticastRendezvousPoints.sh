...
#
# Ref: https://developer.cisco.com/meraki/api-latest/#!get-network-switch-routing-multicast-rendezvous-points
...
function getNetworkSwitchRoutingMulticastRendezvousPoints ()
{
	...
	curl --silent --location --request GET --url "${meraki_uri}/networks/${1}/switch/routing/multicast/rendezvousPoints" --header "Content-Type: application/json" --header "Accept: application/json" --header "X-Cisco-Meraki-API-Key: ${auth_key}"
    fi
}
