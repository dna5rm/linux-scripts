...
function createNetworkCameraQualityRetentionProfile ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Creates new quality retention profile for this network.
	Ref: https://developer.cisco.com/meraki/api-v1/#!create-network-camera-quality-retention-profile
	---
	...
	EOF
        ...
        echo curl --silent --output /dev/null --write-out "%{http_code}\n" --location \
          --request POST --url "${meraki_uri}/networks/${1}/camera/qualityRetentionProfiles" \
