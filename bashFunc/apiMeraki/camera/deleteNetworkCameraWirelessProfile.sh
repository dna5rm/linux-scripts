...
function deleteNetworkCameraWirelessProfile ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete an existing camera wireless profile for this network.
	Ref: https://developer.cisco.com/meraki/api-v1/#!delete-network-camera-wireless-profile
	---
	...
	EOF
        ...
        echo curl --silent --output /dev/null --write-out "%{http_code}\n" --location \
          --request DELETE --url "${meraki_uri}/networks/${1}/camera/wirelessProfiles/${2}" \
