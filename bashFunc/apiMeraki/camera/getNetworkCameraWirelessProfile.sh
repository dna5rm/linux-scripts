...
function getNetworkCameraWirelessProfile ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Retrieve a single camera wireless profile.
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-camera-wireless-profile
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/camera/wirelessProfiles/${2}" \
