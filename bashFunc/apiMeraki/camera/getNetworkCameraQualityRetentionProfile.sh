...
function getNetworkCameraQualityRetentionProfile ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Retrieve a single quality retention profile
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-camera-quality-retention-profile
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/camera/qualityRetentionProfiles/${2}" \
