...
function getOrganizationDevices ()
{
	...
	curl --silent --location --request GET --url "${meraki_uri}/organizations/$1/devices" --header "Content-Type: application/json" --header "Accept: application/json" --header "X-Cisco-Meraki-API-Key: ${auth_key}"
    fi
}
