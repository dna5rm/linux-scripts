...
function getNetworkClientsApplicationUsage ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return the application usage data for clients
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-clients-application-usage
	---
	...
	ssidNumber: An SSID number to include. If not specified, eveusage histories application usagents for all SSIDs will be returned.
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/clients/applicationUsage" \
