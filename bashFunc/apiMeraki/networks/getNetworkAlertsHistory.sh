...
function getNetworkAlertsHistory ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return the alert history for this network
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-alerts-history
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/alerts/history" \
