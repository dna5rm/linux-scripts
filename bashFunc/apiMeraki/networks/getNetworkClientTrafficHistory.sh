...
function getNetworkClientTrafficHistory ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return the client's network traffic data over time
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-client-traffic-history
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/clients/${2}/trafficHistory" \
