...
function deleteNetworkFirmwareUpgradesStagedGroup ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete a Staged Upgrade Group
	Ref: https://developer.cisco.com/meraki/api-v1/#!delete-network-firmware-upgrades-staged-group
	---
	...
	EOF
        ...
        echo curl --silent --output /dev/null --write-out "%{http_code}\n" --location \
          --request DELETE --url "${meraki_uri}/networks/${1}/firmwareUpgrades/staged/groups/${2}" \
