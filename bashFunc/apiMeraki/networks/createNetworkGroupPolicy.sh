...
function createNetworkGroupPolicy ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create a group policy
	Ref: https://developer.cisco.com/meraki/api-v1/#!create-network-group-policy
	---
	...
	EOF
        ...
        echo curl --silent --output /dev/null --write-out "%{http_code}\n" --location \
          --request POST --url "${meraki_uri}/networks/${1}/groupPolicies" \
