...
function getNetworkWebhooksPayloadTemplate ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get the webhook payload template for a network
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-webhooks-payload-template
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/webhooks/payloadTemplates/${2}" \
