...
function getNetworkClientsBandwidthUsageHistory ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Returns a timeseries of total traffic consumption rates for all clients on a network within a given timespan, in megabits per second.
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-clients-bandwidth-usage-history
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/clients/bandwidthUsageHistory" \
