...
function updateNetworkFirmwareUpgrades ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update firmware upgrade information for a network
	Ref: https://developer.cisco.com/meraki/api-v1/#!update-network-firmware-upgrades
	---
	...
	EOF
        ...
        echo curl --silent --output /dev/null --write-out "%{http_code}\n" --location \
          --request PUT --url "${meraki_uri}/networks/${1}/firmwareUpgrades" \
