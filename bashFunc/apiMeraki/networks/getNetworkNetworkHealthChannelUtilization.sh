...
function getNetworkNetworkHealthChannelUtilization ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get the channel utilization over each radio for all APs in a network.
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-network-health-channel-utilization
	---
	...
	perPage: The number of entries per page returned. Acceptable range is 3 - 100. Default is 10.
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/networkHealth/channelUtilization" \
