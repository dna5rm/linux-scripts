...
function getNetworkPiiSmDevicesForKey ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Given a piece of Personally Identifiable Information (PII), return the Systems Manager device ID(s) associated with that identifier
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-pii-sm-devices-for-key
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/pii/smDevicesForKey" \
