...
# Return a network
#
# Ref: https://developer.cisco.com/meraki/api-v1/#!get-network
...
function getNetwork ()
{
	...
	curl --silent --location --request GET --url "${meraki_uri}/networks/${1}" --header "Content-Type: application/json" --header "Accept: application/json" --header "X-Cisco-Meraki-API-Key: ${auth_key}"
    fi
}
