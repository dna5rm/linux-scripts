...
function getOrganizationClientsSearch ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return the client details in an organization
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-organization-clients-search
	---
	...
	perPage: The number of entries per page returned. Acceptable range is 3 - 5. Default is 5.
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/organizations/${1}/clients/search" \
