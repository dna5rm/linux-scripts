...
function getOrganizationUplinksStatuses ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - List the uplink status of every Meraki MX, MG and Z series devices in the organization
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-organization-uplinks-statuses
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/organizations/${1}/uplinks/statuses" \
