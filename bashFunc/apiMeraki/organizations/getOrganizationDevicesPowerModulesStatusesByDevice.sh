...
function getOrganizationDevicesPowerModulesStatusesByDevice ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - List the power status information for devices in an organization
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-organization-devices-power-modules-statuses-by-device
	---
	...
	networkIds: Optional parameter to filter device availabilities by network ID. This filter uses multiple exact matches.
	productTypes: Optional parameter to filter device availabilities by device product types. This filter uses multiple exact matches.
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/organizations/${1}/devices/powerModules/statuses/byDevice" \
