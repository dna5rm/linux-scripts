...
function getOrganizationSummaryTopAppliancesByUtilization ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return the top 10 appliances sorted by utilization over given time range.
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-organization-summary-top-appliances-by-utilization
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/organizations/${1}/summary/top/appliances/byUtilization" \
