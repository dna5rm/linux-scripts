...
function getOrganizationPolicyObjects ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Lists Policy Objects belonging to the organization.
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-organization-policy-objects
	---
	...
	perPage: The number of entries per page returned. Acceptable range is 10 - 5000. Default is 5000.
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/organizations/${1}/policyObjects" \
