...
function getOrganizationSummaryTopDevicesByUsage ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return metrics for organization's top 10 devices sorted by data usage over given time range
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/organizations/${1}/summary/top/devices/byUsage" \
