...
function getOrganizationSummaryTopClientsByUsage ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return metrics for organization's top 10 clients by data usage (in mb) over given time range.
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-organization-summary-top-clients-by-usage
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/organizations/${1}/summary/top/clients/byUsage" \
