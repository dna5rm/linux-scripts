...
function deleteOrganizationConfigTemplate ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Remove a configuration template
	Ref: https://developer.cisco.com/meraki/api-v1/#!delete-organization-config-template
	---
	...
	EOF
        ...
        echo curl --silent --output /dev/null --write-out "%{http_code}\n" --location \
          --request DELETE --url "${meraki_uri}/organizations/${1}/configTemplates/${2}" \
