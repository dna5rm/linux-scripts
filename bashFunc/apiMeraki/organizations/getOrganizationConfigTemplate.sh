...
function getOrganizationConfigTemplate ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return a single configuration template
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-organization-config-template
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/organizations/${1}/configTemplates/${2}" \
