...
function updateOrganizationPolicyObjectsGroup ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Updates a Policy Object Group.
	Ref: https://developer.cisco.com/meraki/api-v1/#!update-organization-policy-objects-group
	---
	...
	EOF
        ...
        echo curl --silent --output /dev/null --write-out "%{http_code}\n" --location \
          --request PUT --url "${meraki_uri}/organizations/${1}/policyObjects/groups/${2}" \
