...
function getOrganizationSummaryTopSsidsByUsage ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return metrics for organization's top 10 ssids by data usage over given time range
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-organization-summary-top-ssids-by-usage
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/organizations/${1}/summary/top/ssids/byUsage" \
