...
function getOrganizationClientsOverview ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return summary information around client data usage (in mb) across the given organization.
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-organization-clients-overview
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/organizations/${1}/clients/overview" \
