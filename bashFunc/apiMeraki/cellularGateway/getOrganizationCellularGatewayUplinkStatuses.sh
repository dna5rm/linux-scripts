...
function getOrganizationCellularGatewayUplinkStatuses ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - List the uplink status of every Meraki MG cellular gateway in the organization
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-organization-cellular-gateway-uplink-statuses
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/organizations/${1}/cellularGateway/uplink/statuses" \
