...
#
# Ref: https://developer.cisco.com/meraki/api-latest/#!get-network-switch-dhcp-server-policy-arp-inspection-trusted-servers
...
function getNetworkSwitchDhcpServerPolicyArpInspectionTrustedServers ()
{
	...
	curl --silent --location --request GET --url "${meraki_uri}/networks/${1}/switch/dhcpServerPolicy/arpInspection/trustedServers" --header "Content-Type: application/json" --header "Accept: application/json" --header "X-Cisco-Meraki-API-Key: ${auth_key}"
    fi
}
