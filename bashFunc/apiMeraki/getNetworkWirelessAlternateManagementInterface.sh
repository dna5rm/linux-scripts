...
#
# Ref: https://developer.cisco.com/meraki/api-latest/#!get-network-wireless-alternate-management-interface
...
function getNetworkWirelessAlternateManagementInterface ()
{
	...
	curl --silent --location --request GET --url "${meraki_uri}/networks/${1}/wireless/alternateManagementInterface" --header "Content-Type: application/json" --header "Accept: application/json" --header "X-Cisco-Meraki-API-Key: ${auth_key}"
    fi
}
