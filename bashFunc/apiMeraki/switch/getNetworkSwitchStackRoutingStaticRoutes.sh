...
function getNetworkSwitchStackRoutingStaticRoutes ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - List layer 3 static routes for a switch stack
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-switch-stack-routing-static-routes
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/switch/stacks/${2}/routing/staticRoutes" \
