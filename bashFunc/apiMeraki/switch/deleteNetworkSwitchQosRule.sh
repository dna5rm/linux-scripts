...
function deleteNetworkSwitchQosRule ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete a quality of service rule
	Ref: https://developer.cisco.com/meraki/api-v1/#!delete-network-switch-qos-rule
	---
	...
	EOF
        ...
        echo curl --silent --output /dev/null --write-out "%{http_code}\n" --location \
          --request DELETE --url "${meraki_uri}/networks/${1}/switch/qosRules/${2}" \
