...
function getNetworkSwitchDhcpV4ServersSeen ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return the network's DHCPv4 servers seen within the selected timeframe (default 1 day)
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-switch-dhcp-v-4-servers-seen
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/switch/dhcp/v4/servers/seen" \
