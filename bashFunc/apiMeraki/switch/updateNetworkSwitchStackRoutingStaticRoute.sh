...
function updateNetworkSwitchStackRoutingStaticRoute ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update a layer 3 static route for a switch stack
	Ref: https://developer.cisco.com/meraki/api-v1/#!update-network-switch-stack-routing-static-route
	---
	...
	EOF
        ...
        echo curl --silent --output /dev/null --write-out "%{http_code}\n" --location \
          --request PUT --url "${meraki_uri}/networks/${1}/switch/stacks/${2}/routing/staticRoutes/${3}" \
