...
function getNetworkSwitchStackRoutingInterface ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return a layer 3 interface from a switch stack
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-switch-stack-routing-interface
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/switch/stacks/${2}/routing/interfaces/${3}" \
