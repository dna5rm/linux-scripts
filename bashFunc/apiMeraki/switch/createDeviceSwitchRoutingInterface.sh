...
function createDeviceSwitchRoutingInterface ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create a layer 3 interface for a switch
	Ref: https://developer.cisco.com/meraki/api-v1/#!create-device-switch-routing-interface
	---
	...
	EOF
        ...
        echo curl --silent --output /dev/null --write-out "%{http_code}\n" --location \
          --request POST --url "${meraki_uri}/devices/${1}/switch/routing/interfaces" \
