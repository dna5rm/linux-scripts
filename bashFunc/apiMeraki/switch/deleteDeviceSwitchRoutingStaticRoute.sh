...
function deleteDeviceSwitchRoutingStaticRoute ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete a layer 3 static route for a switch
	Ref: https://developer.cisco.com/meraki/api-v1/#!delete-device-switch-routing-static-route
	---
	...
	EOF
        ...
        echo curl --silent --output /dev/null --write-out "%{http_code}\n" --location \
          --request DELETE --url "${meraki_uri}/devices/${1}/switch/routing/staticRoutes/${2}" \
