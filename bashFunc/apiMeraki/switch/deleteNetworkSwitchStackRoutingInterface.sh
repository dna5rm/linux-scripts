...
function deleteNetworkSwitchStackRoutingInterface ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete a layer 3 interface from a switch stack
	Ref: https://developer.cisco.com/meraki/api-v1/#!delete-network-switch-stack-routing-interface
	---
	...
	EOF
        ...
        echo curl --silent --output /dev/null --write-out "%{http_code}\n" --location \
          --request DELETE --url "${meraki_uri}/networks/${1}/switch/stacks/${2}/routing/interfaces/${3}" \
