...
function addNetworkSwitchStack ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Add a switch to a stack
	Ref: https://developer.cisco.com/meraki/api-v1/#!add-network-switch-stack
	---
	...
	EOF
        ...
        echo curl --silent --output /dev/null --write-out "%{http_code}\n" --location \
          --request POST --url "${meraki_uri}/networks/${1}/switch/stacks/${2}/add" \
