...
function getNetworkSwitchDhcpServerPolicyArpInspectionWarningsByDevice ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Return the devices that have a Dynamic ARP Inspection warning and their warnings
	Ref: https://developer.cisco.com/meraki/api-v1/#!get-network-switch-dhcp-server-policy-arp-inspection-warnings-by-device
	---
	...
	EOF
         ...
         curl --silent --location \
          --request GET --url "${meraki_uri}/networks/${1}/switch/dhcpServerPolicy/arpInspection/warnings/byDevice" \
