...
function deleteNetworkSwitchAccessPolicy ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete an access policy for a switch network
	Ref: https://developer.cisco.com/meraki/api-v1/#!delete-network-switch-access-policy
	---
	...
	EOF
        ...
        echo curl --silent --output /dev/null --write-out "%{http_code}\n" --location \
          --request DELETE --url "${meraki_uri}/networks/${1}/switch/accessPolicies/${2}" \
