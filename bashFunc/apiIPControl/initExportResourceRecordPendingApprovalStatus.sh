## Export resource record pending approval status
...
function initExportResourceRecordPendingApprovalStatus ()
{
	...
	EOF
	 ...
	 --data-urlencode "ipAddress=${1}" --url "${ipcontrol_uri}/Exports/initExportResourceRecordPendingApprovalStatus"
   fi
}
