## Import a domain resource record
# The importZoneResourceRecord operation enables you to import resource records into IPControl that are not bound to a device, but still appear in a zone file. It can also be used to modify an existing resource record by specifying its id.
#
# Ref: https://${ipcontrol_uri}/inc-rest/api/docs/index.html

function importDomainResourceRecord ()
{
	...
	EOF
	 ...
	 --data-urlencode "ipAddress=${1}" --url "${ipcontrol_uri}/Imports/importDomainResourceRecord"
   fi
}
