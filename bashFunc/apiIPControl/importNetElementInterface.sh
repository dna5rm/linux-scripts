## Import a network element interface
# The importNetworkElement operation enables you to import a network element interface to IPControl. The API also supports updating a network element interface that already exists in the system by specifying its id.
#
# Ref: https://${ipcontrol_uri}/inc-rest/api/docs/index.html

function importNetElementInterface ()
{
	...
	EOF
	 ...
	 --data-urlencode "ipAddress=${1}" --url "${ipcontrol_uri}/Imports/importNetElementInterface"
   fi
}
