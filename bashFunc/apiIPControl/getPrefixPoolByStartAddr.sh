## Get a prefix pool by identifying its IP address
...
function getPrefixPoolByStartAddr ()
{
	...
	EOF
	 ...
	 --data-urlencode "ipAddress=${1}" --url "${ipcontrol_uri}/Gets/getPrefixPoolByStartAddr"
   fi
}
