## Export devices
# The exportDevice operation enables you to export devices from IPControl. Before invoking the exportDevice operation, you must use initExportDevice to initialize the API. The response returned from the init operation becomes the input to this operation.
#
...
function exportDevice ()
{
	...
	EOF
	 ...
	 --data-urlencode "ipAddress=${1}" --url "${ipcontrol_uri}/Exports/exportDevice"
   fi
}
