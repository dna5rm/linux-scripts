## Restore a deleted resource record
# The restoreDeletedResourceRecord operation enables you to restore a deleted resource record. To obtain the required id, use the exportResourceRecordRestoreList operation to export a list of the resource records that were deleted and are available to be restored.
#
# Ref: https://${ipcontrol_uri}/inc-rest/api/docs/index.html

function restoreDeletedResourceRecord ()
{
	...
	EOF
	 ...
	 --data-urlencode "ipAddress=${1}" --url "${ipcontrol_uri}/Imports/restoreDeletedResourceRecord"
   fi
}
