## Get a DHCP server by identifying its IP address
...
function getDhcpServerByIpAddress ()
{
	...
	EOF
	 ...
	 --data-urlencode "ipAddress=${1}" --url "${ipcontrol_uri}/Gets/getDhcpServerByIpAddress"
   fi
}
