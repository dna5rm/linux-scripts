## Get a Network Element by identifying its IP address
...
function getNetworkElementByNameOrIpAddress ()
{
	...
	EOF
	 ...
	 --data-urlencode "ipAddress=${1}" --url "${ipcontrol_uri}/Gets/getNetworkElementByNameOrIpAddress"
   fi
}
