...
function getEdgeHealthSeries ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Fetch system resource metric time series data   
	Ref: /api/sdwan/v2/enterprises/{enterpriseLogicalId}/edges/{edgeLogicalId}/healthStats/timeSeries
	---
	...
	EOF
        ...
        url=( "${vco_uri%/*/*}/api/sdwan/v2/enterprises/${1}/edges/${2}/healthStats/timeSeries" )
