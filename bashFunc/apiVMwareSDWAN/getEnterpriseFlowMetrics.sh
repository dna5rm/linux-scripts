...
# /api/sdwan/v2/enterprises/{enterpriseLogicalId}/flowStats

function getEnterpriseFlowMetrics ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Fetch Customer-global network flow summary statistics   
	Ref: /api/sdwan/v2/enterprises/{enterpriseLogicalId}/flowStats
	---
	...
	EOF
        ...
        url=( "${vco_uri%/*/*}/api/sdwan/v2/enterprises/${1}/flowStats" )
