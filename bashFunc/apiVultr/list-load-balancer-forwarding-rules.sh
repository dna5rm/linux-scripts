...
function list-load-balancer-forwarding-rules ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - List the fowarding rules for a Load Balancer.
	...
	EOF
        ...
        curl --silent --location --request GET --url "${VULTR_API_URI}/load-balancers/${1}/forwarding-rules" \
