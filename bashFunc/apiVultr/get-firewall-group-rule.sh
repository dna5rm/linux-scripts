...
function get-firewall-group-rule ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get a Firewall Rule.
	...
	EOF
        ...
        curl --silent --location --request GET --url "${VULTR_API_URI}/firewalls/${1}/rules/${2}" \
