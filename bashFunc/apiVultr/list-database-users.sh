...
function list-database-users ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - List all database users within the Managed Database.
	...
	EOF
        ...
        curl --silent --location --request GET --url "${VULTR_API_URI}/databases/${1}/users" \
