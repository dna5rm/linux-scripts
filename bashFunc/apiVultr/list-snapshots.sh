...
function list-snapshots ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get information about all Snapshots in your account.
	Ref: /snapshots
	---
	...
	description     query   false   false    the list of Snapshots by `description`
	...
	EOF
        ...
        curl --silent --location --request GET --url "${VULTR_API_URI}/snapshots" \
