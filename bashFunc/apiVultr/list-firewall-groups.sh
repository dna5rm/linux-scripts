...
function list-firewall-groups ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get a list of all Firewall Groups.
	Ref: /firewalls
	---
	...
	EOF
        ...
        curl --silent --location --request GET --url "${VULTR_API_URI}/firewalls" \
