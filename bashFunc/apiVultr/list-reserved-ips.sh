...
function list-reserved-ips ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - List all Reserved IPs in your account.
	Ref: /reserved-ips
	---
	...
	EOF
        ...
        curl --silent --location --request GET --url "${VULTR_API_URI}/reserved-ips" \
