## Get OS
# Get the OS images available for installation at Vultr.
#
...
function get_os ()
{
    ...
    if [[ -z "${vultr_uri}" ]] || [[ -z "${vultr_auth}" ]]; then
        boxText "$(basename "${0}"):${FUNCNAME[0]} - Get OS"
        ...
        curlOutput="$(mktemp)"
        http_code=`curl --silent --location --request GET --url "${vultr_uri}/os" \
        ...
        [[ "${http_code}" != "200" ]] && {
            ...
            [[ -z "${1}" ]] && {
                ...
                jq --arg family "${1}" '[.os[] | select(.family == $family)]' "${curlOutput}"
            }
            rm "${curlOutput}"
        }
