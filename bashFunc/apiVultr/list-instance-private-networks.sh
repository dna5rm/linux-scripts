...
function list-instance-private-networks ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - **Deprecated**: use [List Instance VPCs](#operation/list-instance-vpcs) instead.<br><br>List the private networks for an Instance.
	Ref: /instances/{instance-id}/private-networks
	---
	...
	EOF
        ...
        curl --silent --location --request GET --url "${VULTR_API_URI}/instances/${1}/private-networks" \
