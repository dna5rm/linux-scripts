...
function list-load-balancers ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - List the Load Balancers in your account.
	Ref: /load-balancers
	---
	...
	EOF
        ...
        curl --silent --location --request GET --url "${VULTR_API_URI}/load-balancers" \
