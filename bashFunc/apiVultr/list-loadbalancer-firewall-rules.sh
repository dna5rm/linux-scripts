...
function list-loadbalancer-firewall-rules ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - List the firewall rules for a Load Balancer.
	Ref: /load-balancers/{loadbalancer-id}/firewall-rules
	---
	...
	EOF
        ...
        curl --silent --location --request GET --url "${VULTR_API_URI}/load-balancers/${1}/firewall-rules" \
