...
function delete-load-balancer-forwarding-rule ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete a Forwarding Rule on a Load Balancer.
	...
	EOF
        ...
        curl --silent --location --request DELETE --url "${VULTR_API_URI}/load-balancers/${1}/forwarding-rules/${2}" \
