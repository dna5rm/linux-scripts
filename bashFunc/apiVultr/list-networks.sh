...
function list-networks ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get a list of all Private Networks in your account.<br><br>**Deprecated**: Use [List VPCs](#operation/list-vpcs) instead.
	...
	EOF
        ...
        curl --silent --location --request GET --url "${VULTR_API_URI}/private-networks" \
