...
function delete-nodepool ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete a NodePool from a Kubernetes Cluster
	...
	EOF
        ...
        curl --silent --location --request DELETE --url "${VULTR_API_URI}/kubernetes/clusters/${1}/node-pools/${2}" \
