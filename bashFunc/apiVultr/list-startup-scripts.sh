...
function list-startup-scripts ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get a list of all Startup Scripts.
	Ref: /startup-scripts
	---
	...
	EOF
        ...
        curl --silent --location --request GET --url "${VULTR_API_URI}/startup-scripts" \
