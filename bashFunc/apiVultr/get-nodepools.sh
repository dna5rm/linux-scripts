...
function get-nodepools ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - List all available NodePools on a Kubernetes Cluster
	...
	EOF
        ...
        curl --silent --location --request GET --url "${VULTR_API_URI}/kubernetes/clusters/${1}/node-pools" \
