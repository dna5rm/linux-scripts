...
function role_delete_role_customization ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete role customization
	Ref: /role/deleteRoleCustomization
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/role/deleteRoleCustomization" \
