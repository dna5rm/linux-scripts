...
function get_functional_roles_by_msp ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get functional roles by an Msp
	Ref: /functionalRole/getByEnterpriseProxy
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/functionalRole/getByEnterpriseProxy" \
