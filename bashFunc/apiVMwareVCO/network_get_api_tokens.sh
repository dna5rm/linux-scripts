...
function network_get_api_tokens ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get API Tokens of a network
	Ref: /network/getApiTokens
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/network/getApiTokens" \
