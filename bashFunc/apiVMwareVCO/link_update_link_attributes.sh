...
function link_update_link_attributes ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update WAN link
	Ref: /link/updateLinkAttributes
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/link/updateLinkAttributes" \
