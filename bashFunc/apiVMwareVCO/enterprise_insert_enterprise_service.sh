...
function enterprise_insert_enterprise_service ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create enterprise service
	Ref: /enterprise/insertEnterpriseService
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/insertEnterpriseService" \
