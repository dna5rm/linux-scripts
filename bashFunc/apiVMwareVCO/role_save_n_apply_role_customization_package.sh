...
function role_save_n_apply_role_customization_package ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update and Apply Role Customization Package
	Ref: /role/saveAndApplyRoleCustomizationPackage
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/role/saveAndApplyRoleCustomizationPackage" \
