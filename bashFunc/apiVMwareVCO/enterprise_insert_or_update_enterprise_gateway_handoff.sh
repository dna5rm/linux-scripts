...
function enterprise_insert_or_update_enterprise_gateway_handoff ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create or update enterprise gateway handoff configuration
	Ref: /enterprise/insertOrUpdateEnterpriseGatewayHandoff
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/insertOrUpdateEnterpriseGatewayHandoff" \
