...
function enterprise_proxy_insert_enterprise_proxy_user ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create partner admin user
	Ref: /enterpriseProxy/insertEnterpriseProxyUser
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/insertEnterpriseProxyUser" \
