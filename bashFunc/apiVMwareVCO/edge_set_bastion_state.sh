...
function edge_set_bastion_state ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Set the edge bastion state
	Ref: /edge/setEdgeBastionState
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/edge/setEdgeBastionState" \
