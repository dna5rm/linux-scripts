...
function migration_get_migration_actions ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get migration actions from migration entity records filtered by enterpriseId,edgeId,gatewayId ,enterpriseObjectId or type
	Ref: /migration/getMigrationActions
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/migration/getMigrationActions" \
