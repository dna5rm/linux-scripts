...
function system_property_get_system_property ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get system property
	Ref: /systemProperty/getSystemProperty
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/systemProperty/getSystemProperty" \
