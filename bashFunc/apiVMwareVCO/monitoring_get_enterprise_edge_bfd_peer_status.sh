...
function monitoring_get_enterprise_edge_bfd_peer_status ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get Edge BFD peer status for all enterprise Edges
	Ref: /monitoring/getEnterpriseEdgeBfdPeerStatus
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/monitoring/getEnterpriseEdgeBfdPeerStatus" \
