...
function edge_edge_request_reactivation ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Prepare Edge for reactivation
	Ref: /edge/edgeRequestReactivation
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/edge/edgeRequestReactivation" \
