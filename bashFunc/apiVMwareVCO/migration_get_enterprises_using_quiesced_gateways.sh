...
function migration_get_enterprises_using_quiesced_gateways ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Gets a list of enterpriseIds using quiesced gateways
	Ref: /migration/getEnterprisesUsingQuiescedGateways
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/migration/getEnterprisesUsingQuiescedGateways" \
