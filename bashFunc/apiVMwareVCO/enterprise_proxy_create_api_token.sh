...
function enterprise_proxy_create_api_token ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create API Token
	Ref: /enterpriseProxy/createApiToken
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/createApiToken" \
