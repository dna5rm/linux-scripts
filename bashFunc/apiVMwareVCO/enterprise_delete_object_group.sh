...
function enterprise_delete_object_group ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete an enterprise object group
	Ref: /enterprise/deleteObjectGroup
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/deleteObjectGroup" \
