...
function enterprise_user_delete_enterprise_user ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete enterprise user
	Ref: /enterpriseUser/deleteEnterpriseUser
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseUser/deleteEnterpriseUser" \
