...
function enterprise_get_enterprise_alert_configurations ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise alert configuration(s)
	Ref: /enterprise/getEnterpriseAlertConfigurations
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseAlertConfigurations" \
