...
function enterprise_proxy_get_enterprise_proxy_enterprises ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get partner enterprises
	Ref: /enterpriseProxy/getEnterpriseProxyEnterprises
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/getEnterpriseProxyEnterprises" \
