...
function monitoring_get_enterprise_edge_link_status ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get Edge and link status data
	Ref: /monitoring/getEnterpriseEdgeLinkStatus
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/monitoring/getEnterpriseEdgeLinkStatus" \
