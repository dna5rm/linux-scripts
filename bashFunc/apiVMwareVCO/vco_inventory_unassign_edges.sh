...
# /vcoInventory/unassignEdges

function vco_inventory_unassign_edges ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Unassign Edge inventory
	Ref: /vcoInventory/unassignEdges
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/vcoInventory/unassignEdges" \
