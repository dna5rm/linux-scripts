...
function enterprise_get_enterprise_distributed_cost_calculation ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get distributed cost calculation property for an enterprise
	Ref: /enterprise/getEnterpriseDistributedCostCalculation
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseDistributedCostCalculation" \
