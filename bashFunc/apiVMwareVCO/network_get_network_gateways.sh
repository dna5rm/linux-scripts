...
function network_get_network_gateways ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get gateways on network
	Ref: /network/getNetworkGateways
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/network/getNetworkGateways" \
