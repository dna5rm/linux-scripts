...
function enterprise_proxy_get_enterprise_proxy_user ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise proxy user
	Ref: /enterpriseProxy/getEnterpriseProxyUser
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/getEnterpriseProxyUser" \
