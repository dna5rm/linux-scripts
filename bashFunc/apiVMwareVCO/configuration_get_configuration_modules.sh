...
function configuration_get_configuration_modules ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get configuration profile modules
	Ref: /configuration/getConfigurationModules
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/configuration/getConfigurationModules" \
