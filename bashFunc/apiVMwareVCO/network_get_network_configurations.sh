...
function network_get_network_configurations ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get operator configuration profiles
	Ref: /network/getNetworkConfigurations
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/network/getNetworkConfigurations" \
