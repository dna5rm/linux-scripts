...
function enterprise_get_enterprise_specific_operator_configs ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Gets the enterprise assigned operator configurations
	Ref: /enterprise/getEnterpriseSpecificOperatorConfigurations
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseSpecificOperatorConfigurations" \
