...
function gateway_property_delete_replacement_gateway_property ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete replacement gateway property
	Ref: /gatewayProperty/deleteReplacementGateway
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/gatewayProperty/deleteReplacementGateway" \
