...
function import_ssh_key ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Import users ssh key
	Ref: /sshConfig/importSshKey
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/sshConfig/importSshKey" \
