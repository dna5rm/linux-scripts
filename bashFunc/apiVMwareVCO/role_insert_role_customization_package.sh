...
function role_insert_role_customization_package ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Inserts Role Customization Package
	Ref: /role/insertRoleCustomizationPackage
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/role/insertRoleCustomizationPackage" \
