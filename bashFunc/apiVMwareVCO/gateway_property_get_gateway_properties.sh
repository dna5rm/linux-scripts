...
function gateway_property_get_gateway_properties ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get gateway properties
	Ref: /gatewayProperty/getGatewayProperties
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/gatewayProperty/getGatewayProperties" \
