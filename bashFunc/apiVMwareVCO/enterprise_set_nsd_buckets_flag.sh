...
function enterprise_set_nsd_buckets_flag ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Set NSDBuckets flag for an enterprise.
	Ref: /enterprise/setEnterpriseUseNSDPolicy
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/setEnterpriseUseNSDPolicy" \
