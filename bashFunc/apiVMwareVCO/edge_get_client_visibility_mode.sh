...
function edge_get_client_visibility_mode ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get an edge's client visibility mode
	Ref: /edge/getClientVisibilityMode
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/edge/getClientVisibilityMode" \
