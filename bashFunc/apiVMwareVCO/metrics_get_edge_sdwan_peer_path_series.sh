...
function metrics_get_edge_sdwan_peer_path_series ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get path stats metric time series data for a given edge and SD-WAN peer pair
	Ref: /metrics/getEdgeSDWANPeerPathSeries
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/metrics/getEdgeSDWANPeerPathSeries" \
