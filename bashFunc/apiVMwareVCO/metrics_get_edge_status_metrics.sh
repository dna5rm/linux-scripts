...
function metrics_get_edge_status_metrics ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get Edge healthStats metrics for an interval
	Ref: /metrics/getEdgeStatusMetrics
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/metrics/getEdgeStatusMetrics" \
