...
function metrics_get_gateway_status_metrics ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get Gateway health metric summaries for an interval
	Ref: /metrics/getGatewayStatusMetrics
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/metrics/getGatewayStatusMetrics" \
