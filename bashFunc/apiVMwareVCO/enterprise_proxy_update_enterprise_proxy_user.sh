...
function enterprise_proxy_update_enterprise_proxy_user ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update enterprise proxy admin user
	Ref: /enterpriseProxy/updateEnterpriseProxyUser
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/updateEnterpriseProxyUser" \
