...
function get_role_customization_privileges ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get privileges for roles in a Role Customization Package
	Ref: /role/getRoleCustomizationPrivileges
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/role/getRoleCustomizationPrivileges" \
