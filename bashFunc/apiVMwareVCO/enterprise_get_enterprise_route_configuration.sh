...
function enterprise_get_enterprise_route_configuration ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise global routing preferences
	Ref: /enterprise/getEnterpriseRouteConfiguration
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseRouteConfiguration" \
