...
function get_functional_roles_by_enterprise ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get functional roles by an Enterprise
	Ref: /functionalRole/getByEnterprise
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/functionalRole/getByEnterprise" \
