...
function gateway_gateway_provision ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Provision gateway
	Ref: /gateway/gatewayProvision
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/gateway/gatewayProvision" \
