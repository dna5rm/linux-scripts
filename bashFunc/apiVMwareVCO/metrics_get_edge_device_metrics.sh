...
function metrics_get_edge_device_metrics ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get flow metric aggregate data by client device
	Ref: /metrics/getEdgeDeviceMetrics
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/metrics/getEdgeDeviceMetrics" \
