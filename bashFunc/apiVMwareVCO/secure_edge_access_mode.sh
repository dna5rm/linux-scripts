...
function secure_edge_access_mode ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Insert or Update  Secure Edge Access Mode
	Ref: /enterprise/insertOrUpdateSecureEdgeAccessMode
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/insertOrUpdateSecureEdgeAccessMode" \
