...
function enterprise_get_enterprise_all_alert_recipients ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get recipients receiving all enterprise alerts
	Ref: /enterprise/getEnterpriseAllAlertsRecipients
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseAllAlertsRecipients" \
