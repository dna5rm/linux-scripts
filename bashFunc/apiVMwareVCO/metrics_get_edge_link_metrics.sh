...
function metrics_get_edge_link_metrics ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get summary link metrics for an Edge
	Ref: /metrics/getEdgeLinkMetrics
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/metrics/getEdgeLinkMetrics" \
