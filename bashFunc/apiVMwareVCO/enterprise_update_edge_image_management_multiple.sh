...
function enterprise_update_edge_image_management_multiple ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Updates manage software image feature for multiple customers
	Ref: /enterprise/updateEdgeImageManagementForEnterprises
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/updateEdgeImageManagementForEnterprises" \
