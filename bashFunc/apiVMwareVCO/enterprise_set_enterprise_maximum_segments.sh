...
function enterprise_set_enterprise_maximum_segments ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Set enterprise maximum segments
	Ref: /enterprise/setEnterpriseMaximumSegments
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/setEnterpriseMaximumSegments" \
