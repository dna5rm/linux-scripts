...
function configuration_update_configuration_module ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update configuration module
	Ref: /configuration/updateConfigurationModule
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/configuration/updateConfigurationModule" \
