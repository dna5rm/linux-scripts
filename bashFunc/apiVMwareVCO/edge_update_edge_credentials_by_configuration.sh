...
function edge_update_edge_credentials_by_configuration ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update Edge local UI credentials by configuration profile
	Ref: /edge/updateEdgeCredentialsByConfiguration
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/edge/updateEdgeCredentialsByConfiguration" \
