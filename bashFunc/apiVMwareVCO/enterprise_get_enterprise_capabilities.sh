...
function enterprise_get_enterprise_capabilities ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise capabilities
	Ref: /enterprise/getEnterpriseCapabilities
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseCapabilities" \
