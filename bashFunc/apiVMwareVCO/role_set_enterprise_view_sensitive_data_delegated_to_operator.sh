...
function role_set_enterprise_view_sensitive_data_delegated_to_operator ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Grant enterprise view sensitive data access to network operator
	Ref: /role/setEnterpriseViewSensitiveDataDelegatedToOperator
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/role/setEnterpriseViewSensitiveDataDelegatedToOperator" \
