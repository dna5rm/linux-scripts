...
function enterprise_get_enterprise_edges ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise Edges
	Ref: /enterprise/getEnterpriseEdges
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseEdges" \
