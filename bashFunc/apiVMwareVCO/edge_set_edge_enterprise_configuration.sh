...
function edge_set_edge_enterprise_configuration ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Apply configuration profile to Edge
	Ref: /edge/setEdgeEnterpriseConfiguration
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/edge/setEdgeEnterpriseConfiguration" \
