...
function role_get_eligible_privileges_for_customization ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Gets list of privileges that are eligible for customization
	Ref: /role/getEligiblePrivilegesForCustomization
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/role/getEligiblePrivilegesForCustomization" \
