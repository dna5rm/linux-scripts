...
function monitoring_get_vpn_site_status ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Gets the VPN site status for automated deployments
	Ref: /monitoring/getVPNSiteStatus
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/monitoring/getVPNSiteStatus" \
