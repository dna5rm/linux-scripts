...
function disaster_recovery_get_replication_status ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get disaster recovery status
	Ref: /disasterRecovery/getReplicationStatus
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/disasterRecovery/getReplicationStatus" \
