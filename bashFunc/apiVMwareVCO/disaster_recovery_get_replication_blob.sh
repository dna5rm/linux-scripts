...
function disaster_recovery_get_replication_blob ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get blob needed to configure VCO replication on standby
	Ref: /disasterRecovery/getReplicationBlob
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/disasterRecovery/getReplicationBlob" \
