...
function enterprise_update_edge_image_management ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Updates manage software image feature for a customer
	Ref: /enterprise/updateEdgeImageManagement
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/updateEdgeImageManagement" \
