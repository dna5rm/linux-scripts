...
function enterprise_get_enterprise_network_allocations ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise network allocations
	Ref: /enterprise/getEnterpriseNetworkAllocations
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseNetworkAllocations" \
