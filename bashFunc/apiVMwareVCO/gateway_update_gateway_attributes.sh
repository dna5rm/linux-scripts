...
function gateway_update_gateway_attributes ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update gateway attributes
	Ref: /gateway/updateGatewayAttributes
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/gateway/updateGatewayAttributes" \
