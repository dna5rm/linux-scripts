...
function set_default_enterprise_operator_configuration ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Makes a particular operator profile, the default for an enterprise
	Ref: /enterprise/setDefaultEnterpriseOperatorConfiguration
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/setDefaultEnterpriseOperatorConfiguration" \
