...
function gateway_get_gateway_edge_assignments ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get edge assignments for a gateway
	Ref: /gateway/getGatewayEdgeAssignments
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/gateway/getGatewayEdgeAssignments" \
