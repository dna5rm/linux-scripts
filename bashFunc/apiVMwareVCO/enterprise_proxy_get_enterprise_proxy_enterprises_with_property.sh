...
function enterprise_proxy_get_enterprise_proxy_enterprises_with_property ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Gets all MSP enterprises having a particular property and a value
	Ref: /enterprise/getEnterpriseProxyEnterprisesWithProperty
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseProxyEnterprisesWithProperty" \
