...
function enterprise_enable_cluster_for_edge_hub ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Configure cluster as a VPN hub
	Ref: /enterprise/enableClusterForEdgeHub
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/enableClusterForEdgeHub" \
