...
function edge_get_edge_sdwan_peers ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get an edge's SD-WAN peers
	Ref: /edge/getEdgeSDWANPeers
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/edge/getEdgeSDWANPeers" \
