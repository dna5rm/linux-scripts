...
function enterprise_encode_enterprise_key ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Encrypt an enterprise key
	Ref: /enterprise/encodeEnterpriseKey
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/encodeEnterpriseKey" \
