...
function get_composite_roles_by_operator ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get composite roles by operator
	Ref: /compositeRole/getByOperator
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/compositeRole/getByOperator" \
