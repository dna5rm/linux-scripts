...
function configuration_update_application_map_profiles ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Refresh Application Map and push updates to Edges
	Ref: /configuration/updateApplicationMapProfiles
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/configuration/updateApplicationMapProfiles" \
