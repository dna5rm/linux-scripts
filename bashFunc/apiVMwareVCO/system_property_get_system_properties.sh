...
function system_property_get_system_properties ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get all system properties
	Ref: /systemProperty/getSystemProperties
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/systemProperty/getSystemProperties" \
