...
function metrics_get_enterprise_flow_metrics ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get flow metric aggregate data by specified flow dimension
	Ref: /metrics/getEnterpriseFlowMetrics
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/metrics/getEnterpriseFlowMetrics" \
