...
function enterprise_insert_enterprise_network_segment ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create enterprise network segment
	Ref: /enterprise/insertEnterpriseNetworkSegment
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/insertEnterpriseNetworkSegment" \
