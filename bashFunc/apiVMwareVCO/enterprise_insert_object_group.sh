...
function enterprise_insert_object_group ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create enterprise object group
	Ref: /enterprise/insertObjectGroup
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/insertObjectGroup" \
