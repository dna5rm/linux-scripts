...
function enterprise_get_enterprises_missinggateway ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Gets the list of Enterprises with missing Gateway Type
	Ref: /enterprise/getEnterprisesMissingGateway
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterprisesMissingGateway" \
