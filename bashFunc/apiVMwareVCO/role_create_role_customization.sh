...
function role_create_role_customization ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create role customization
	Ref: /role/createRoleCustomization
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/role/createRoleCustomization" \
