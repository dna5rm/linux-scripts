...
function set_client_device_host_name ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Set hostname for client device
	Ref: /clientDevice/setClientDeviceHostName
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/clientDevice/setClientDeviceHostName" \
