...
function network_insert_network_gateway_pool ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create gateway pool
	Ref: /network/insertNetworkGatewayPool
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/network/insertNetworkGatewayPool" \
