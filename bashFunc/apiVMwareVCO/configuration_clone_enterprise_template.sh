...
function configuration_clone_enterprise_template ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Clone default enterprise configuration profile
	Ref: /configuration/cloneEnterpriseTemplate
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/configuration/cloneEnterpriseTemplate" \
