...
function enterprise_get_enterprise_alert_definitions ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise alert definitions
	Ref: /enterprise/getEnterpriseAlertDefinitions
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseAlertDefinitions" \
