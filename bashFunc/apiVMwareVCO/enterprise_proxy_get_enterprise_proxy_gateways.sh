...
function enterprise_proxy_get_enterprise_proxy_gateways ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get gateways for enterprise proxy
	Ref: /enterpriseProxy/getEnterpriseProxyGateways
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/getEnterpriseProxyGateways" \
