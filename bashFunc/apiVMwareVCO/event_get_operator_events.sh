...
function event_get_operator_events ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get operator events
	Ref: /event/getOperatorEvents
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/event/getOperatorEvents" \
