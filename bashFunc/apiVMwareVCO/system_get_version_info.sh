...
function system_get_version_info ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get system version information
	Ref: /system/getVersionInfo
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/system/getVersionInfo" \
