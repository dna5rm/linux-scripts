...
function enterprise_set_bastion_state ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Set the enterprise bastion state
	Ref: /enterprise/setEnterpriseBastionState
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/setEnterpriseBastionState" \
