...
function enterprise_set_enterprise_distributed_cost_calculation ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Set distributed cost calculation for an enterprise.
	Ref: /enterprise/setEnterpriseDistributedCostCalculation
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/setEnterpriseDistributedCostCalculation" \
