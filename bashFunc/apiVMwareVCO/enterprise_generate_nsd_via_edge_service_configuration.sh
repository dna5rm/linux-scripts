...
function enterprise_generate_nsd_via_edge_service_configuration ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get Enterprise Default Edge Direct Provider Service Data
	Ref: /enterprise/generateNsdViaEdgeServiceConfiguration
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/generateNsdViaEdgeServiceConfiguration" \
