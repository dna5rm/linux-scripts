...
function network_set_replacement_gateway ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Insert/Update replacement gateway
	Ref: /gatewayProperty/setReplacementGateway
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/gatewayProperty/setReplacementGateway" \
