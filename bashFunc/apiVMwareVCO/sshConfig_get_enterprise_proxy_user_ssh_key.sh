...
function sshConfig_get_enterprise_proxy_user_ssh_key ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get SSH Keys
	Ref: /sshConfig/getEnterpriseProxyUserSSHKey
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/sshConfig/getEnterpriseProxyUserSSHKey" \
