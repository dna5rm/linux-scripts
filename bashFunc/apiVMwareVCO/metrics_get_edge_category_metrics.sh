...
function metrics_get_edge_category_metrics ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get flow metric aggregate data by application category
	Ref: /metrics/getEdgeCategoryMetrics
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/metrics/getEdgeCategoryMetrics" \
