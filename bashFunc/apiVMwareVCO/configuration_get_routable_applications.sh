...
function configuration_get_routable_applications ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get first packet routable applications
	Ref: /configuration/getRoutableApplications
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/configuration/getRoutableApplications" \
