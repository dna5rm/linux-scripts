...
function role_apply_role_customization_package ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Apply Role Customization Package
	Ref: /role/applyRoleCustomizationPackage
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/role/applyRoleCustomizationPackage" \
