...
function role_remove_custom_roles ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Removes role customizations at the given level (NETWORK, PARTNER, ENTERPRISE or ALL) in VCO
	Ref: /role/removeCustomRoles
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/role/removeCustomRoles" \
