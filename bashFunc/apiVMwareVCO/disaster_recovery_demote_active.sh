...
function disaster_recovery_demote_active ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Demote current VCO from active to zombie
	Ref: /disasterRecovery/demoteActive
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/disasterRecovery/demoteActive" \
