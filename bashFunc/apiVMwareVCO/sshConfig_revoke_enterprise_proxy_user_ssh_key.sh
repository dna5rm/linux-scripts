...
function sshConfig_revoke_enterprise_proxy_user_ssh_key ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Revoke enterprise proxy users SSH Keys
	Ref: /sshConfig/revokeEnterpriseProxyUserSSHKey
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/sshConfig/revokeEnterpriseProxyUserSSHKey" \
