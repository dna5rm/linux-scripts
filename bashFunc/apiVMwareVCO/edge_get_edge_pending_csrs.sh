...
# /pki/getEdgePendingCSRs

function edge_get_edge_pending_csrs ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get all edge pending CSRs
	Ref: /pki/getEdgePendingCSRs
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/pki/getEdgePendingCSRs" \
