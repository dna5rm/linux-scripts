...
function enterprise_proxy_delete_enterprise_proxy_user ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete partner admin user
	Ref: /enterpriseProxy/deleteEnterpriseProxyUser
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/deleteEnterpriseProxyUser" \
