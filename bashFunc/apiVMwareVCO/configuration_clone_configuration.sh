...
function configuration_clone_configuration ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Clone configuration profile
	Ref: /configuration/cloneConfiguration
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/configuration/cloneConfiguration" \
