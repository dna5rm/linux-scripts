...
function monitoring_get_aggregate_edge_link_metrics ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get aggregate Edge link metrics across enterprises
	Ref: /monitoring/getAggregateEdgeLinkMetrics
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/monitoring/getAggregateEdgeLinkMetrics" \
