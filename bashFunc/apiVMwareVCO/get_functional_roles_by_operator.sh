...
function get_functional_roles_by_operator ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get functional roles by an Operator
	Ref: /functionalRole/getByOperator
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/functionalRole/getByOperator" \
