...
# /gatewayPool/addGatewayPoolGateway

function gateway_pool_add_gateway_pool_gateway ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Add Gateway Pool Gateway
	Ref: /gatewayPool/addGatewayPoolGateway
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/gatewayPool/addGatewayPoolGateway" \
