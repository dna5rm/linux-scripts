...
function vco_diagnostics_get_vco_db_diagnostics ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get VCO Database Diagnostics
	Ref: /vcoDiagnostics/getVcoDbDiagnostics
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/vcoDiagnostics/getVcoDbDiagnostics" \
