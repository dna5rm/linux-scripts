...
function get_composite_roles_by_msp ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get composite roles by an Msp
	Ref: /compositeRole/getByEnterpriseProxy
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/compositeRole/getByEnterpriseProxy" \
