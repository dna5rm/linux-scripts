...
function enterprise_update_enterprise_network_allocation ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update enterprise network allocation
	Ref: /enterprise/updateEnterpriseNetworkAllocation
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/updateEnterpriseNetworkAllocation" \
