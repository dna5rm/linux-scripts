...
function network_delete_network_gateway_pool ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete gateway pool
	Ref: /network/deleteNetworkGatewayPool
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/network/deleteNetworkGatewayPool" \
