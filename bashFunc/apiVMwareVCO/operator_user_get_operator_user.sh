...
function operator_user_get_operator_user ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get operator user
	Ref: /operatorUser/getOperatorUser
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/operatorUser/getOperatorUser" \
