...
function system_property_update_system_property ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update system property
	Ref: /systemProperty/updateSystemProperty
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/systemProperty/updateSystemProperty" \
