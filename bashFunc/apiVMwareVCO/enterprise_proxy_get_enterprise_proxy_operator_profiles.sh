...
function enterprise_proxy_get_enterprise_proxy_operator_profiles ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get partner operator profiles
	Ref: /enterpriseProxy/getEnterpriseProxyOperatorProfiles
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/getEnterpriseProxyOperatorProfiles" \
