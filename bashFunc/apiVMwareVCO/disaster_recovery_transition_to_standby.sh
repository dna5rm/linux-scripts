...
function disaster_recovery_transition_to_standby ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Transition VCO to standby
	Ref: /disasterRecovery/transitionToStandby
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/disasterRecovery/transitionToStandby" \
