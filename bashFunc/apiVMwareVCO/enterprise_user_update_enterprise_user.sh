...
function enterprise_user_update_enterprise_user ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update enterprise user
	Ref: /enterpriseUser/updateEnterpriseUser
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseUser/updateEnterpriseUser" \
