...
function enterprise_get_enterprise_rate_limits ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise rate limits
	Ref: /enterprise/getEnterpriseRateLimits
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseRateLimits" \
