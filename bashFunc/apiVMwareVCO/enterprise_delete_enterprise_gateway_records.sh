...
function enterprise_delete_enterprise_gateway_records ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete gateway BGP neighbor record(s) for enterprise
	Ref: /enterprise/deleteEnterpriseGatewayRecords
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/deleteEnterpriseGatewayRecords" \
