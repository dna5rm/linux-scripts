...
function edge_update_analytics_settings_for_edges ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update Edge Analytics Settings
	Ref: /edge/updateAnalyticsSettingsForEdges
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/edge/updateAnalyticsSettingsForEdges" \
