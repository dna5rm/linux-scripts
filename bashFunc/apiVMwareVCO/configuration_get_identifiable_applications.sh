...
function configuration_get_identifiable_applications ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get the applications that are identifiable through DPI.
	Ref: /configuration/getIdentifiableApplications
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/configuration/getIdentifiableApplications" \
