...
function metrics_get_edge_link_series ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get link metric time series data for an Edge
	Ref: /metrics/getEdgeLinkSeries
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/metrics/getEdgeLinkSeries" \
