...
function link_quality_event_get_link_quality_events ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get link quality (QoE) scores for an Edge
	Ref: /linkQualityEvent/getLinkQualityEvents
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/linkQualityEvent/getLinkQualityEvents" \
