...
function enterprise_set_enterprise_refresh_routes_version ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Set refresh routes version to latest
	Ref: /enterprise/recalcRefreshRoutesVersion
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/recalcRefreshRoutesVersion" \
