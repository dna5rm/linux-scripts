...
function role_set_enterprise_delegated_to_enterprise_proxy ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Grant enterprise access to partner
	Ref: /role/setEnterpriseDelegatedToEnterpriseProxy
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/role/setEnterpriseDelegatedToEnterpriseProxy" \
