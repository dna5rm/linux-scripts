...
function role_set_enterprise_user_management_delegated_to_operator ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Grant enterprise user access to network operator
	Ref: /role/setEnterpriseUserManagementDelegatedToOperator
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/role/setEnterpriseUserManagementDelegatedToOperator" \
