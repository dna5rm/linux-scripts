...
function edge_update_edge_attributes ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update Edge attributes
	Ref: /edge/updateEdgeAttributes
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/edge/updateEdgeAttributes" \
