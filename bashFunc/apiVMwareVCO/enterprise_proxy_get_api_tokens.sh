...
function enterprise_proxy_get_api_tokens ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get API Tokens of a network, enterprise or enterprise Proxy
	Ref: /enterpriseProxy/getApiTokens
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/getApiTokens" \
