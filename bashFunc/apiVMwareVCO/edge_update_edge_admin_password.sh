...
function edge_update_edge_admin_password ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update Edge local UI authentication credentials
	Ref: /edge/updateEdgeAdminPassword
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/edge/updateEdgeAdminPassword" \
