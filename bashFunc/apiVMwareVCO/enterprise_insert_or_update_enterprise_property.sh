...
function enterprise_insert_or_update_enterprise_property ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Insert or update an enterprise property
	Ref: /enterprise/insertOrUpdateEnterpriseProperty
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/insertOrUpdateEnterpriseProperty" \
