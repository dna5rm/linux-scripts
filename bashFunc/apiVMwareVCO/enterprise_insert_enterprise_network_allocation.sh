...
function enterprise_insert_enterprise_network_allocation ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create enterprise network allocation
	Ref: /enterprise/insertEnterpriseNetworkAllocation
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/insertEnterpriseNetworkAllocation" \
