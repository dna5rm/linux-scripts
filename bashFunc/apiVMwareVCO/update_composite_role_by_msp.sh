...
function update_composite_role_by_msp ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Updates a composite role by an Msp
	Ref: /compositeRole/updateByEnterpriseProxy
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/compositeRole/updateByEnterpriseProxy" \
