...
function enterprise_proxy_insert_or_update_enterprise_specific_rate_limits ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Set enterprise proxy rate limits
	Ref: /enterprise/insertOrUpdateEnterpriseProxyRateLimits
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/insertOrUpdateEnterpriseProxyRateLimits" \
