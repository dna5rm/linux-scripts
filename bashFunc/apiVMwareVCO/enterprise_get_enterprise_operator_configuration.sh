...
function enterprise_get_enterprise_operator_configuration ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get the Operator Profile assigned to an Enterprise
	Ref: /enterprise/getEnterpriseOperatorConfiguration
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseOperatorConfiguration" \
