...
function enterprise_proxy_insert_or_update_enterprise_proxy_capability ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create or update partner capability
	Ref: /enterpriseProxy/insertOrUpdateEnterpriseProxyCapability
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/insertOrUpdateEnterpriseProxyCapability" \
