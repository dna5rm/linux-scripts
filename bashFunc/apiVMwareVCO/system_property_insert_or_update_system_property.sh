...
function system_property_insert_or_update_system_property ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create or update system property
	Ref: /systemProperty/insertOrUpdateSystemProperty
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/systemProperty/insertOrUpdateSystemProperty" \
