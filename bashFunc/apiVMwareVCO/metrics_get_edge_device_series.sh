...
function metrics_get_edge_device_series ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get flow metric time series data by client device
	Ref: /metrics/getEdgeDeviceSeries
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/metrics/getEdgeDeviceSeries" \
