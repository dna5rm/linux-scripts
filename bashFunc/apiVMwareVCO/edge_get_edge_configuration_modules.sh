...
function edge_get_edge_configuration_modules ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get effective Edge configuration
	Ref: /edge/getEdgeConfigurationModules
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/edge/getEdgeConfigurationModules" \
