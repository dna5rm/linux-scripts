...
function create_composite_role_by_msp ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create composite role by an MSP
	Ref: /compositeRole/createByEnterpriseProxy
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/compositeRole/createByEnterpriseProxy" \
