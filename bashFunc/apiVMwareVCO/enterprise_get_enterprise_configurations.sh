...
function enterprise_get_enterprise_configurations ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise configuration profiles
	Ref: /enterprise/getEnterpriseConfigurations
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseConfigurations" \
