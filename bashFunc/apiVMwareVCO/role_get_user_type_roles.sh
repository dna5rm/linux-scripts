...
function role_get_user_type_roles ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get roles per user type
	Ref: /role/getUserTypeRoles
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/role/getUserTypeRoles" \
