...
function edge_set_edge_operator_configuration ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Apply an Operator Profile to an Edge
	Ref: /edge/setEdgeOperatorConfiguration
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/edge/setEdgeOperatorConfiguration" \
