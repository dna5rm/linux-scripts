...
function firewall_get_enterprise_firewall_logs ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise firewall logs
	Ref: /firewall/getEnterpriseFirewallLogs
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/firewall/getEnterpriseFirewallLogs" \
