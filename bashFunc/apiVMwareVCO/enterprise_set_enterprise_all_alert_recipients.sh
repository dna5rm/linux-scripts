...
function enterprise_set_enterprise_all_alert_recipients ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Specify recipients for all enterprise alerts
	Ref: /enterprise/setEnterpriseAllAlertsRecipients
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/setEnterpriseAllAlertsRecipients" \
