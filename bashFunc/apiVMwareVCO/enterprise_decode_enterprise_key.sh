...
function enterprise_decode_enterprise_key ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Decrypt an enterprise key
	Ref: /enterprise/decodeEnterpriseKey
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/decodeEnterpriseKey" \
