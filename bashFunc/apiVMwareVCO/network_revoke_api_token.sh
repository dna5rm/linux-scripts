...
function network_revoke_api_token ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Revoke an Operator API Token
	Ref: /network/revokeApiToken
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/network/revokeApiToken" \
