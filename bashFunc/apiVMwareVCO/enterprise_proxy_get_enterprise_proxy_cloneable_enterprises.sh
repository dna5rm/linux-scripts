...
function enterprise_proxy_get_enterprise_proxy_cloneable_enterprises ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get cloneable enterprises under enterprise proxy 
	Ref: /enterpriseProxy/getEnterpriseProxyCloneableEnterprises
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/getEnterpriseProxyCloneableEnterprises" \
