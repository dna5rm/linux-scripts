...
function enterprise_insert_or_update_analytics_configuration ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Inserts/updates analytics configuration of an enterprise
	Ref: /enterprise/insertOrUpdateAnalyticsConfiguration
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/insertOrUpdateAnalyticsConfiguration" \
