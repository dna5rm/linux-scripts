...
function migration_get_migration_gateways ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get NSD migration gateways
	Ref: /migration/getMigrationGateways
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/migration/getMigrationGateways" \
