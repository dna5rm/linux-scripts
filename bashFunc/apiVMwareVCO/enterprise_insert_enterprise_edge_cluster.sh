...
function enterprise_insert_enterprise_edge_cluster ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Insert enterprise edge cluster 
	Ref: /enterprise/insertEnterpriseEdgeCluster
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/insertEnterpriseEdgeCluster" \
