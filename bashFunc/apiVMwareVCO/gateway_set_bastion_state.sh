...
function gateway_set_bastion_state ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Set the gateway bastion state
	Ref: /gateway/setGatewayBastionState
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/gateway/setGatewayBastionState" \
