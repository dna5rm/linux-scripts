...
function enterprise_user_get_enterprise_user ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise user
	Ref: /enterpriseUser/getEnterpriseUser
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseUser/getEnterpriseUser" \
