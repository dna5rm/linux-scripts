...
function metrics_get_edge_dest_series ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get flow metric time series data by destination
	Ref: /metrics/getEdgeDestSeries
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/metrics/getEdgeDestSeries" \
