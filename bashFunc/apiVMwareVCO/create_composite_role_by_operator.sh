...
function create_composite_role_by_operator ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create composite role by an Operator
	Ref: /compositeRole/createByOperator
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/compositeRole/createByOperator" \
