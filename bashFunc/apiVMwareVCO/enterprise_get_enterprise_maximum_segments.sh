...
function enterprise_get_enterprise_maximum_segments ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise maximum segment
	Ref: /enterprise/getEnterpriseMaximumSegments
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseMaximumSegments" \
