...
function enterprise_delete_enterprise_network_segment ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete an enterprise network segment
	Ref: /enterprise/deleteEnterpriseNetworkSegment
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/deleteEnterpriseNetworkSegment" \
