...
function event_get_proxy_events ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Fetch Partner events
	Ref: /event/getProxyEvents
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/event/getProxyEvents" \
