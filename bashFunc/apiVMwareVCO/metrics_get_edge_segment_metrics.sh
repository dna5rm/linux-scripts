...
function metrics_get_edge_segment_metrics ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get flow metric aggregate data by segment Id
	Ref: /metrics/getEdgeSegmentMetrics
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/metrics/getEdgeSegmentMetrics" \
