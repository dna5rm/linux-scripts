...
function configuration_insert_configuration_module ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create configuration module
	Ref: /configuration/insertConfigurationModule
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/configuration/insertConfigurationModule" \
