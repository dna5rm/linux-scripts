...
function enterprise_proxy_get_enterpriseProxy_rate_limits ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterpriseProxy rate limits
	Ref: /enterpriseProxy/getEnterpriseProxyRateLimits
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/getEnterpriseProxyRateLimits" \
