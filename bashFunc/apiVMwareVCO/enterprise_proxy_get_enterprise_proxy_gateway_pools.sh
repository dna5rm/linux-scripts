...
function enterprise_proxy_get_enterprise_proxy_gateway_pools ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get gateway pools
	Ref: /enterpriseProxy/getEnterpriseProxyGatewayPools
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/getEnterpriseProxyGatewayPools" \
