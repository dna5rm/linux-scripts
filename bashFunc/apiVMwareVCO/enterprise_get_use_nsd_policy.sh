...
function enterprise_get_use_nsd_policy ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get Use NSD policy property of an enterprise
	Ref: /enterprise/getEnterpriseUseNSDPolicy
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseUseNSDPolicy" \
