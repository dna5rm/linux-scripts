...
function enterprise_update_enterprise_route_configuration ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update enterprise global routing preferences
	Ref: /enterprise/updateEnterpriseRouteConfiguration
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/updateEnterpriseRouteConfiguration" \
