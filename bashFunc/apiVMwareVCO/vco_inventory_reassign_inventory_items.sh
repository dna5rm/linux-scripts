...
function vco_inventory_reassign_inventory_items ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Partner edge reassignment
	Ref: /vcoInventory/reassignInventoryItems
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/vcoInventory/reassignInventoryItems" \
