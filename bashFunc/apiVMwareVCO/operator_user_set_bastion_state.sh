...
function operator_user_set_bastion_state ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Set the operatorUser bastion state
	Ref: /operatorUser/setOperatorUserBastionState
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/operatorUser/setOperatorUserBastionState" \
