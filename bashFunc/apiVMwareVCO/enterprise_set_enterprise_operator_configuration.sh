...
function enterprise_set_enterprise_operator_configuration ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Apply operator profile to given enterprise
	Ref: /enterprise/setEnterpriseOperatorConfiguration
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/setEnterpriseOperatorConfiguration" \
