...
function generate_ssh_key ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Generate users ssh key
	Ref: /sshConfig/generateSshKey
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/sshConfig/generateSshKey" \
