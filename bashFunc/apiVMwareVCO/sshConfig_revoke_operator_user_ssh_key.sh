...
function sshConfig_revoke_operator_user_ssh_key ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Revoke operator users SSH Keys
	Ref: /sshConfig/revokeOperatorUserSSHKey
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/sshConfig/revokeOperatorUserSSHKey" \
