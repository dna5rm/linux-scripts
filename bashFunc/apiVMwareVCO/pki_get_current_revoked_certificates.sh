...
# /pki/getCurrentRevokedCertificates

function pki_get_current_revoked_certificates ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get revoked edge and gateway certificates
	Ref: /pki/getCurrentRevokedCertificates
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/pki/getCurrentRevokedCertificates" \
