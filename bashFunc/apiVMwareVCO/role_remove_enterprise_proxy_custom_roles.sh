...
function role_remove_enterprise_proxy_custom_roles ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Removes role customizations at PARTNER level
	Ref: /role/removeEnterpriseProxyCustomRoles
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/role/removeEnterpriseProxyCustomRoles" \
