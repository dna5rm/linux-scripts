...
function enterprise_get_enterprise_alerts ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get triggered enterprise alerts
	Ref: /enterprise/getEnterpriseAlerts
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseAlerts" \
