...
# /pki/getGatewayPendingCSRs

function gateway_get_gateway_pending_csrs ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get all gateway pending CSRs
	Ref: /pki/getGatewayPendingCSRs
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/pki/getGatewayPendingCSRs" \
