...
function enterprise_proxy_download_api_token ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Download/Generate API Token
	Ref: /enterpriseProxy/downloadApiToken
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/downloadApiToken" \
