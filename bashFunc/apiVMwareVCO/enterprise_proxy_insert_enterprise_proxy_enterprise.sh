...
function enterprise_proxy_insert_enterprise_proxy_enterprise ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create enterprise owned by MSP
	Ref: /enterpriseProxy/insertEnterpriseProxyEnterprise
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/insertEnterpriseProxyEnterprise" \
