...
function metrics_get_edge_app_link_series ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get flow metric time series data
	Ref: /metrics/getEdgeAppLinkSeries
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/metrics/getEdgeAppLinkSeries" \
