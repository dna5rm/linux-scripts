...
function enterprise_create_api_token ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create Enterprise API Token
	Ref: /enterprise/createApiToken
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/createApiToken" \
