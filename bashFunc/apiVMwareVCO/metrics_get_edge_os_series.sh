...
function metrics_get_edge_os_series ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get flow metric time series data by client OS
	Ref: /metrics/getEdgeOsSeries
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/metrics/getEdgeOsSeries" \
