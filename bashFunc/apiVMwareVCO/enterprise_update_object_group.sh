...
function enterprise_update_object_group ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update an enterprise object group
	Ref: /enterprise/updateObjectGroup
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/updateObjectGroup" \
