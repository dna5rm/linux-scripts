...
function role_set_enterprise_proxy_delegated_to_operator ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Grant enterprise proxy access to network operator
	Ref: /role/setEnterpriseProxyDelegatedToOperator
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/role/setEnterpriseProxyDelegatedToOperator" \
