...
function sshConfig_revoke_enterprise_user_ssh_key ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Revoke enterprise users SSH Keys
	Ref: /sshConfig/revokeEnterpriseUserSSHKey
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/sshConfig/revokeEnterpriseUserSSHKey" \
