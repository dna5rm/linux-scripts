...
# /pki/insertGatewayCertificate

function gateway_insert_gateway_certificate ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Insert gateway certificate
	Ref: /pki/insertGatewayCertificate
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/pki/insertGatewayCertificate" \
