...
function enterprise_update_enterprise_security_policy ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update enterprise security policy
	Ref: /enterprise/updateEnterpriseSecurityPolicy
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/updateEnterpriseSecurityPolicy" \
