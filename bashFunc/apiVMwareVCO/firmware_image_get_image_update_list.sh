...
function firmware_image_get_image_update_list ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get image lists
	Ref: /softwareUpdate/getFirmwareUpdatesList
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/softwareUpdate/getFirmwareUpdatesList" \
