...
function operator_user_delete_operator_user ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete operator user
	Ref: /operatorUser/deleteOperatorUser
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/operatorUser/deleteOperatorUser" \
