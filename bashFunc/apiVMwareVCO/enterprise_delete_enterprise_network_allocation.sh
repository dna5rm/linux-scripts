...
function enterprise_delete_enterprise_network_allocation ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete enterprise network allocation
	Ref: /enterprise/deleteEnterpriseNetworkAllocation
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/deleteEnterpriseNetworkAllocation" \
