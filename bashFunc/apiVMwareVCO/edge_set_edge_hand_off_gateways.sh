...
function edge_set_edge_hand_off_gateways ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Set Edge on premise hand-off gateway(s)
	Ref: /edge/setEdgeHandOffGateways
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/edge/setEdgeHandOffGateways" \
