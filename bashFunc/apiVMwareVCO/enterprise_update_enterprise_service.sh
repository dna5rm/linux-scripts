...
function enterprise_update_enterprise_service ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update enterprise network service
	Ref: /enterprise/updateEnterpriseService
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/updateEnterpriseService" \
