...
function enterprise_insert_or_update_enterprise_specific_rate_limits ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Set enterprise rate limits
	Ref: /enterprise/insertOrUpdateEnterpriseRateLimits
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/insertOrUpdateEnterpriseRateLimits" \
