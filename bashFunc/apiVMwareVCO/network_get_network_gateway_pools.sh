...
function network_get_network_gateway_pools ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get gateway pools on network
	Ref: /network/getNetworkGatewayPools
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/network/getNetworkGatewayPools" \
