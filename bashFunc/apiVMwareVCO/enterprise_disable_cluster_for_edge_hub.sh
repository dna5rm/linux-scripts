...
function enterprise_disable_cluster_for_edge_hub ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Unconfigure cluster as a VPN hub
	Ref: /enterprise/disableClusterForEdgeHub
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/disableClusterForEdgeHub" \
