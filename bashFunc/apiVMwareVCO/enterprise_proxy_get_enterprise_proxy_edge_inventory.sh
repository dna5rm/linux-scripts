...
function enterprise_proxy_get_enterprise_proxy_edge_inventory ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get partner enterprises and associated Edge inventory
	Ref: /enterpriseProxy/getEnterpriseProxyEdgeInventory
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/getEnterpriseProxyEdgeInventory" \
