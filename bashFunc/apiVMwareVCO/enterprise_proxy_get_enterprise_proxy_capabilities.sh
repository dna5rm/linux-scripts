...
function enterprise_proxy_get_enterprise_proxy_capabilities ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get partner capabilities
	Ref: /enterpriseProxy/getEnterpriseProxyCapabilities
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/getEnterpriseProxyCapabilities" \
