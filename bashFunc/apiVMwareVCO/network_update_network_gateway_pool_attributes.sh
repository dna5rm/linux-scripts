...
function network_update_network_gateway_pool_attributes ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update gateway pool attributes
	Ref: /network/updateNetworkGatewayPoolAttributes
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/network/updateNetworkGatewayPoolAttributes" \
