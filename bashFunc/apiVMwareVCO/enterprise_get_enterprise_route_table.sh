...
function enterprise_get_enterprise_route_table ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise route table
	Ref: /enterprise/getEnterpriseRouteTable
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseRouteTable" \
