...
function event_get_enterprise_events ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get Edge events
	Ref: /event/getEnterpriseEvents
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/event/getEnterpriseEvents" \
