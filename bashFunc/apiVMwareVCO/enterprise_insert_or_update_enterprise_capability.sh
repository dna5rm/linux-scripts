...
function enterprise_insert_or_update_enterprise_capability ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create or update enterprise capability
	Ref: /enterprise/insertOrUpdateEnterpriseCapability
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/insertOrUpdateEnterpriseCapability" \
