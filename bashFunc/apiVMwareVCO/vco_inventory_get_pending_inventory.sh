...
function vco_inventory_get_pending_inventory ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get pending Edge inventory from Maestro
	Ref: /vcoInventory/getPendingInventory
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/vcoInventory/getPendingInventory" \
