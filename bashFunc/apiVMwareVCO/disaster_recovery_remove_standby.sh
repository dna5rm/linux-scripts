...
function disaster_recovery_remove_standby ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Remove VCO disaster recovery on current server
	Ref: /disasterRecovery/removeStandby
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/disasterRecovery/removeStandby" \
