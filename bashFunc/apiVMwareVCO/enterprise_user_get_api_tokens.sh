...
function enterprise_user_get_api_tokens ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get API Tokens owned by current user
	Ref: /enterpriseUser/getApiTokens
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseUser/getApiTokens" \
