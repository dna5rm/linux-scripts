...
function enterprise_update_enterprise_edge_cluster ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update enterprise edge cluster 
	Ref: /enterprise/updateEnterpriseEdgeCluster
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/updateEnterpriseEdgeCluster" \
