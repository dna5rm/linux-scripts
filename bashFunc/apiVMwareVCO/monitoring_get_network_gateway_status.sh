...
function monitoring_get_network_gateway_status ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get Network Gateway monitoring status
	Ref: /monitoring/getNetworkGatewayStatus
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/monitoring/getNetworkGatewayStatus" \
