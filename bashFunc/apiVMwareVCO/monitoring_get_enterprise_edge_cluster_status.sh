...
function monitoring_get_enterprise_edge_cluster_status ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get Edge Cluster status
	Ref: /monitoring/getEnterpriseEdgeClusterStatus
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/monitoring/getEnterpriseEdgeClusterStatus" \
