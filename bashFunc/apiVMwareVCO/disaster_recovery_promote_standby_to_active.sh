...
function disaster_recovery_promote_standby_to_active ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Promote standby VCO to active
	Ref: /disasterRecovery/promoteStandbyToActive
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/disasterRecovery/promoteStandbyToActive" \
