...
function monitoring_get_enterprise_edge_bgp_peer_status ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get Edge BGP peer status for all enterprise Edges
	Ref: /monitoring/getEnterpriseEdgeBgpPeerStatus
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/monitoring/getEnterpriseEdgeBgpPeerStatus" \
