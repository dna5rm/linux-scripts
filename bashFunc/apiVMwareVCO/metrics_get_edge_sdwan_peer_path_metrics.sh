...
function metrics_get_edge_sdwan_peer_path_metrics ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get path stats metric aggregate data for a given edge and SD-WAN peer pair
	Ref: /metrics/getEdgeSDWANPeerPathMetrics
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/metrics/getEdgeSDWANPeerPathMetrics" \
