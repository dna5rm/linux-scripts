...
function network_get_network_cloneable_enterprises ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprises on network
	Ref: /network/getNetworkCloneableEnterprises
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/network/getNetworkCloneableEnterprises" \
