...
function delete_composite_roles_by_msp ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete a composite role
	Ref: /compositeRole/deleteByEnterpriseProxy
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/compositeRole/deleteByEnterpriseProxy" \
