...
function enterprise_proxy_get_enterprise_proxy_property ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise proxy property
	Ref: /enterpriseProxy/getEnterpriseProxyProperty
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterpriseProxy/getEnterpriseProxyProperty" \
