...
function enterprise_insert_or_update_enterprise_alert_configurations ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create, update, or delete enterprise alert configurations
	Ref: /enterprise/insertOrUpdateEnterpriseAlertConfigurations
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/insertOrUpdateEnterpriseAlertConfigurations" \
