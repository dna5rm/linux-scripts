...
function network_get_network_operator_users ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get operator users for network
	Ref: /network/getNetworkOperatorUsers
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/network/getNetworkOperatorUsers" \
