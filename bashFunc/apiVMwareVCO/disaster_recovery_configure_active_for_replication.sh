...
function disaster_recovery_configure_active_for_replication ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Configure current VCO for disaster recovery
	Ref: /disasterRecovery/configureActiveForReplication
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/disasterRecovery/configureActiveForReplication" \
