...
function add_enterprise_operator_configuration ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Adds an operator profile to multiple enterprises
	Ref: /enterprise/addEnterpriseOperatorConfiguration
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/addEnterpriseOperatorConfiguration" \
