...
function enterprise_get_enterprise_gateway_handoff ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise gateway handoff configuration
	Ref: /enterprise/getEnterpriseGatewayHandoff
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseGatewayHandoff" \
