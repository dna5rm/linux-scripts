...
function sshConfig_get_operator_user_ssh_key ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get operator users SSH Keys
	Ref: /sshConfig/getOperatorUserSSHKey
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/sshConfig/getOperatorUserSSHKey" \
