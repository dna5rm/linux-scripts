...
function configuration_clone_and_convert_configuration ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create new segment-based profile from existing network-based profile
	Ref: /configuration/cloneAndConvertConfiguration
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/configuration/cloneAndConvertConfiguration" \
