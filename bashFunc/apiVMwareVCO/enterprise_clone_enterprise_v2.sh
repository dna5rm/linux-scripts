...
function enterprise_clone_enterprise_v2 ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Clone an enterprise, owned by the operator or an MSP.
	Ref: /enterprise/cloneEnterpriseV2
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/cloneEnterpriseV2" \
