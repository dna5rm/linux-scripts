...
function enterprise_update_enterprise ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Update enterprise
	Ref: /enterprise/updateEnterprise
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/updateEnterprise" \
