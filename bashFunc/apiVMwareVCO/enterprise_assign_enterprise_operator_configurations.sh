...
function enterprise_assign_enterprise_operator_configurations ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Assigns one or more operator configurations to an enterprise
	Ref: /enterprise/assignEnterpriseOperatorConfigurations
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/assignEnterpriseOperatorConfigurations" \
