...
function monitoring_get_enterprise_bgp_peer_status ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get gateway BGP peer status for all enterprise gateways
	Ref: /monitoring/getEnterpriseBgpPeerStatus
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/monitoring/getEnterpriseBgpPeerStatus" \
