...
function system_property_insert_system_property ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Create system property
	Ref: /systemProperty/insertSystemProperty
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/systemProperty/insertSystemProperty" \
