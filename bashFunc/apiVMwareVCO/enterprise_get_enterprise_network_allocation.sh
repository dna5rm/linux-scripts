...
function enterprise_get_enterprise_network_allocation ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise network allocation
	Ref: /enterprise/getEnterpriseNetworkAllocation
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getEnterpriseNetworkAllocation" \
