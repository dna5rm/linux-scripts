...
# /pki/insertEdgeCertificate

function edge_insert_edge_certificate ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Insert edge certificate
	Ref: /pki/insertEdgeCertificate
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/pki/insertEdgeCertificate" \
