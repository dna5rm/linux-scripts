...
function vco_inventory_delete_rma_inventory_items ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete RMAed inventory Edges
	Ref: /vcoInventory/deleteRmaInventoryItems
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/vcoInventory/deleteRmaInventoryItems" \
