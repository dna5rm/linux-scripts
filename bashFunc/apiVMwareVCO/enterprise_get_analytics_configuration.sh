...
function enterprise_get_analytics_configuration ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise analytics configuration details
	Ref: /enterprise/getAnalyticsConfiguration
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getAnalyticsConfiguration" \
