...
function vpn_generate_vpn_gateway_configuration ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Provision a non-SD-WAN VPN site
	Ref: /vpn/generateVpnGatewayConfiguration
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/vpn/generateVpnGatewayConfiguration" \
