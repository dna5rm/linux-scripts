...
function migration_verify_nsd_tunnels ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Verify NSD tunnels and transition the state of migration action
	Ref: /migration/verifyNSDTunnels
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/migration/verifyNSDTunnels" \
