...
function gateway_delete_gateway ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Delete a gateway
	Ref: /gateway/deleteGateway
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/gateway/deleteGateway" \
