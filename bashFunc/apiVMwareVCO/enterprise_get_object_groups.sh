...
function enterprise_get_object_groups ()
{
	...
	cat <<-EOF
	$(basename "${0}" 2> /dev/null):${FUNCNAME[0]} - Get enterprise object group(s)
	Ref: /enterprise/getObjectGroups
	---
	...
	EOF
        ...
        curl --silent --insecure --location --request POST --url "${vco_uri}/enterprise/getObjectGroups" \
