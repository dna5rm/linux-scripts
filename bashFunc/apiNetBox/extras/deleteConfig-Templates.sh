## deleteConfig-Templates # null
# /api/extras/config-templates/{id}/

function deleteConfig-Templates ()
{
	...
	Ref: /api/extras/config-templates/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/extras/config-templates/${1}" \
