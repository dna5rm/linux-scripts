## deleteExport-Templates # null
# /api/extras/export-templates/{id}/

function deleteExport-Templates ()
{
	...
	Ref: /api/extras/export-templates/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/extras/export-templates/${1}" \
