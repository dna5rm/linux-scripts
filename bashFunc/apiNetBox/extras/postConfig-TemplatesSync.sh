## postConfig-TemplatesSync # null
# /api/extras/config-templates/{id}/sync/

function postConfig-TemplatesSync ()
{
	...
	Ref: /api/extras/config-templates/{id}/sync/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/extras/config-templates/${1}/sync" \
