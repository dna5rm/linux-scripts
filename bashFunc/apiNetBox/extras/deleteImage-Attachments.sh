## deleteImage-Attachments # null
# /api/extras/image-attachments/{id}/

function deleteImage-Attachments ()
{
	...
	Ref: /api/extras/image-attachments/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/extras/image-attachments/${1}" \
