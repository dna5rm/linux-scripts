## postConfig-ContextsSync # null
# /api/extras/config-contexts/{id}/sync/

function postConfig-ContextsSync ()
{
	...
	Ref: /api/extras/config-contexts/{id}/sync/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/extras/config-contexts/${1}/sync" \
