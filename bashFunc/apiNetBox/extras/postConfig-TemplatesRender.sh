## postConfig-TemplatesRender # null
# /api/extras/config-templates/{id}/render/

function postConfig-TemplatesRender ()
{
	...
	Ref: /api/extras/config-templates/{id}/render/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/extras/config-templates/${1}/render" \
