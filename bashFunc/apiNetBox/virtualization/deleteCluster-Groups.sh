## deleteCluster-Groups # null
# /api/virtualization/cluster-groups/{id}/

function deleteCluster-Groups ()
{
	...
	Ref: /api/virtualization/cluster-groups/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/virtualization/cluster-groups/${1}" \
