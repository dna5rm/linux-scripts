## deleteCluster-Types # null
# /api/virtualization/cluster-types/{id}/

function deleteCluster-Types ()
{
	...
	Ref: /api/virtualization/cluster-types/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/virtualization/cluster-types/${1}" \
