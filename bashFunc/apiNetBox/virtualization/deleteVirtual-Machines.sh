## deleteVirtual-Machines # null
# /api/virtualization/virtual-machines/{id}/

function deleteVirtual-Machines ()
{
	...
	Ref: /api/virtualization/virtual-machines/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/virtualization/virtual-machines/${1}" \
