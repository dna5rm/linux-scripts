## deleteProvider-Networks # null
# /api/circuits/provider-networks/{id}/

function deleteProvider-Networks ()
{
	...
	Ref: /api/circuits/provider-networks/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/circuits/provider-networks/${1}" \
