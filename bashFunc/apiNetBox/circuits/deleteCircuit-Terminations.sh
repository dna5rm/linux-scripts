## deleteCircuit-Terminations # null
# /api/circuits/circuit-terminations/{id}/

function deleteCircuit-Terminations ()
{
	...
	Ref: /api/circuits/circuit-terminations/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/circuits/circuit-terminations/${1}" \
