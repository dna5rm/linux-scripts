## postCircuit-Terminations # null
# /api/circuits/circuit-terminations/

function postCircuit-Terminations ()
{
	...
	Ref: /api/circuits/circuit-terminations/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/circuits/circuit-terminations" \
