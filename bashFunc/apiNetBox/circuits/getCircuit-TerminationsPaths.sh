## getCircuit-TerminationsPaths # null
# /api/circuits/circuit-terminations/{id}/paths/

function getCircuit-TerminationsPaths ()
{
	...
	Ref: /api/circuits/circuit-terminations/{id}/paths/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request GET --url "${netbox_api}/api/circuits/circuit-terminations/${1}/paths" \
