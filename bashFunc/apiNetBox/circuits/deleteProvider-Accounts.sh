## deleteProvider-Accounts # null
# /api/circuits/provider-accounts/{id}/

function deleteProvider-Accounts ()
{
	...
	Ref: /api/circuits/provider-accounts/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/circuits/provider-accounts/${1}" \
