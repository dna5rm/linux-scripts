...
[[ ! -f "_swagger.json" ]] && {
    . ${HOME}/bin/bashFunc/y2j.sh
    curl --silent curl --silent -X GET "http://localhost:8000/api/schema/?format=json" | jq '.' > _swagger.json
}
###################
...
######### Extract variables.
        ...
        output="${method}$(cut -d/ -f4- <<<${path} | sed 's/\/{[^}]*}//g;s/\// /g;s/\b\(.\)/\u\1/g;s/ //g')"
...
######### Generate API function script.
	...
	function ${output} ()
	{
	    ...
	    if [[ -z "\${netbox_api}" ]] || [[ -z "\${netbox_token}" ]]` [[ "$(awk -F"{" '{print NF-1}' <<<${path})" -gt 1 ]] && { echo " || [[ -z \"\\\${$(awk -F"{" '{print NF-1}' <<<${path})}\" ]]"; }`; then
	$(printf "\t")cat <<-EOF
	$(printf "\t")\$(basename "\${0}" 2> /dev/null):\${FUNCNAME[0]} - ${summary}
	$(printf "\t")Ref: ${path:-null}
	$(printf "\t")---
	$(printf "\t")API Base URI: \\\${netbox_api} (\${netbox_api:-required})
	$(printf "\t")Authentication Token: \\\${netbox_token} (\${netbox_token:-required})
	...
	$(printf "\t")EOF
	        ...
	        response=\$(curl --silent --insecure --location --write-out "%{http_code}" \\
	          --request ${method^^} --url "\${netbox_api}/$(for i in $(echo ${path} | sed 's/\// /g'); do [[ "${i:0:1}" != "{" ]] && { echo "${i}"; } || { echo "\${$((${pcount:-0}+1))}"; let ++pcount; }; done | sed -z 's/\n/\//g;s/\/$//')" \\
	          --header "Content-Type: application/json" --header "Authorization: Token \${netbox_token}" \\
	          --data "\${$(( $(awk -F"{" '{print NF-1}' <<<${path}) + 1 )):-{\}}")
