## deleteContact-Assignments # null
# /api/tenancy/contact-assignments/{id}/

function deleteContact-Assignments ()
{
	...
	Ref: /api/tenancy/contact-assignments/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/tenancy/contact-assignments/${1}" \
