## postContact-Assignments # null
# /api/tenancy/contact-assignments/

function postContact-Assignments ()
{
	...
	Ref: /api/tenancy/contact-assignments/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/tenancy/contact-assignments" \
