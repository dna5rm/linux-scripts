## postWireless-Lan-Groups # null
# /api/wireless/wireless-lan-groups/

function postWireless-Lan-Groups ()
{
	...
	Ref: /api/wireless/wireless-lan-groups/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/wireless/wireless-lan-groups" \
