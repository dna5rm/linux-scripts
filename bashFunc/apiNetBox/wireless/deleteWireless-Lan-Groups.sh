## deleteWireless-Lan-Groups # null
# /api/wireless/wireless-lan-groups/{id}/

function deleteWireless-Lan-Groups ()
{
	...
	Ref: /api/wireless/wireless-lan-groups/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/wireless/wireless-lan-groups/${1}" \
