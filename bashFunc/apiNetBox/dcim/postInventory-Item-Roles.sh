## postInventory-Item-Roles # null
# /api/dcim/inventory-item-roles/

function postInventory-Item-Roles ()
{
	...
	Ref: /api/dcim/inventory-item-roles/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/dcim/inventory-item-roles" \
