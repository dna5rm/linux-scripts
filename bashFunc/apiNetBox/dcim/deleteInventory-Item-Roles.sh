## deleteInventory-Item-Roles # null
# /api/dcim/inventory-item-roles/{id}/

function deleteInventory-Item-Roles ()
{
	...
	Ref: /api/dcim/inventory-item-roles/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/dcim/inventory-item-roles/${1}" \
