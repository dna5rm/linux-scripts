## deleteInventory-Item-Templates # null
# /api/dcim/inventory-item-templates/{id}/

function deleteInventory-Item-Templates ()
{
	...
	Ref: /api/dcim/inventory-item-templates/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/dcim/inventory-item-templates/${1}" \
