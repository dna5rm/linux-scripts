## patchConsole-Server-Port-Templates # null
# /api/dcim/console-server-port-templates/{id}/

function patchConsole-Server-Port-Templates ()
{
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request PATCH --url "${netbox_api}/api/dcim/console-server-port-templates/${1}" \
