## deleteConsole-Port-Templates # null
# /api/dcim/console-port-templates/{id}/

function deleteConsole-Port-Templates ()
{
	...
	Ref: /api/dcim/console-port-templates/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/dcim/console-port-templates/${1}" \
