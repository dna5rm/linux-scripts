## getConsole-Server-PortsTrace # null
# /api/dcim/console-server-ports/{id}/trace/

function getConsole-Server-PortsTrace ()
{
	...
	Ref: /api/dcim/console-server-ports/{id}/trace/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request GET --url "${netbox_api}/api/dcim/console-server-ports/${1}/trace" \
