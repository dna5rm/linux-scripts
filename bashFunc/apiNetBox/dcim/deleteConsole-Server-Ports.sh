## deleteConsole-Server-Ports # null
# /api/dcim/console-server-ports/{id}/

function deleteConsole-Server-Ports ()
{
	...
	Ref: /api/dcim/console-server-ports/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/dcim/console-server-ports/${1}" \
