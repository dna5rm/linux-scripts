## deleteRack-Reservations # null
# /api/dcim/rack-reservations/{id}/

function deleteRack-Reservations ()
{
	...
	Ref: /api/dcim/rack-reservations/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/dcim/rack-reservations/${1}" \
