## postVirtual-Device-Contexts # null
# /api/dcim/virtual-device-contexts/

function postVirtual-Device-Contexts ()
{
	...
	Ref: /api/dcim/virtual-device-contexts/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/dcim/virtual-device-contexts" \
