## postDevicesRender-Config # null
# /api/dcim/devices/{id}/render-config/

function postDevicesRender-Config ()
{
	...
	Ref: /api/dcim/devices/{id}/render-config/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/dcim/devices/${1}/render-config" \
