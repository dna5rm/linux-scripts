## postPower-Port-Templates # null
# /api/dcim/power-port-templates/

function postPower-Port-Templates ()
{
	...
	Ref: /api/dcim/power-port-templates/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/dcim/power-port-templates" \
