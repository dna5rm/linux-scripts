## deleteRear-Port-Templates # null
# /api/dcim/rear-port-templates/{id}/

function deleteRear-Port-Templates ()
{
	...
	Ref: /api/dcim/rear-port-templates/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/dcim/rear-port-templates/${1}" \
