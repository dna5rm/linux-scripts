## deleteConsole-Server-Port-Templates # null
# /api/dcim/console-server-port-templates/{id}/

function deleteConsole-Server-Port-Templates ()
{
	...
	Ref: /api/dcim/console-server-port-templates/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/dcim/console-server-port-templates/${1}" \
