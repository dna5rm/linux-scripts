## deleteDevice-Bay-Templates # null
# /api/dcim/device-bay-templates/{id}/

function deleteDevice-Bay-Templates ()
{
	...
	Ref: /api/dcim/device-bay-templates/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/dcim/device-bay-templates/${1}" \
