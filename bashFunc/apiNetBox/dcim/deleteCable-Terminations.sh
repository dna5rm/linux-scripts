## deleteCable-Terminations # null
# /api/dcim/cable-terminations/{id}/

function deleteCable-Terminations ()
{
	...
	Ref: /api/dcim/cable-terminations/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/dcim/cable-terminations/${1}" \
