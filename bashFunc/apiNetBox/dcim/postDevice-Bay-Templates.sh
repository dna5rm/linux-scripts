## postDevice-Bay-Templates # null
# /api/dcim/device-bay-templates/

function postDevice-Bay-Templates ()
{
	...
	Ref: /api/dcim/device-bay-templates/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/dcim/device-bay-templates" \
