## postPower-Outlet-Templates # null
# /api/dcim/power-outlet-templates/

function postPower-Outlet-Templates ()
{
	...
	Ref: /api/dcim/power-outlet-templates/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/dcim/power-outlet-templates" \
