## postConsole-Server-Ports # null
# /api/dcim/console-server-ports/

function postConsole-Server-Ports ()
{
	...
	Ref: /api/dcim/console-server-ports/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/dcim/console-server-ports" \
