## deleteVirtual-Device-Contexts # null
# /api/dcim/virtual-device-contexts/{id}/

function deleteVirtual-Device-Contexts ()
{
	...
	Ref: /api/dcim/virtual-device-contexts/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/dcim/virtual-device-contexts/${1}" \
