## deletePower-Port-Templates # null
# /api/dcim/power-port-templates/{id}/

function deletePower-Port-Templates ()
{
	...
	Ref: /api/dcim/power-port-templates/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/dcim/power-port-templates/${1}" \
