## deletePower-Outlet-Templates # null
# /api/dcim/power-outlet-templates/{id}/

function deletePower-Outlet-Templates ()
{
	...
	Ref: /api/dcim/power-outlet-templates/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/dcim/power-outlet-templates/${1}" \
