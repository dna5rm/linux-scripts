## deleteFront-Port-Templates # null
# /api/dcim/front-port-templates/{id}/

function deleteFront-Port-Templates ()
{
	...
	Ref: /api/dcim/front-port-templates/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/dcim/front-port-templates/${1}" \
