## postConsole-Port-Templates # null
# /api/dcim/console-port-templates/

function postConsole-Port-Templates ()
{
	...
	Ref: /api/dcim/console-port-templates/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/dcim/console-port-templates" \
