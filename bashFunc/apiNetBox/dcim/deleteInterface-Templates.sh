## deleteInterface-Templates # null
# /api/dcim/interface-templates/{id}/

function deleteInterface-Templates ()
{
	...
	Ref: /api/dcim/interface-templates/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/dcim/interface-templates/${1}" \
