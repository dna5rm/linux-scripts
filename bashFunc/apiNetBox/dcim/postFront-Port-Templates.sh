## postFront-Port-Templates # null
# /api/dcim/front-port-templates/

function postFront-Port-Templates ()
{
	...
	Ref: /api/dcim/front-port-templates/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/dcim/front-port-templates" \
