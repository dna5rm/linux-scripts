## postConsole-Server-Port-Templates # null
# /api/dcim/console-server-port-templates/

function postConsole-Server-Port-Templates ()
{
	...
	Ref: /api/dcim/console-server-port-templates/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/dcim/console-server-port-templates" \
