## postInventory-Item-Templates # null
# /api/dcim/inventory-item-templates/

function postInventory-Item-Templates ()
{
	...
	Ref: /api/dcim/inventory-item-templates/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/dcim/inventory-item-templates" \
