## deleteL2vpn-Terminations # null
# /api/ipam/l2vpn-terminations/{id}/

function deleteL2vpn-Terminations ()
{
	...
	Ref: /api/ipam/l2vpn-terminations/{id}/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request DELETE --url "${netbox_api}/api/ipam/l2vpn-terminations/${1}" \
