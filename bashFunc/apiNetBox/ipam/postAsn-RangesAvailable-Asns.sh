## postAsn-RangesAvailable-Asns # null
# /api/ipam/asn-ranges/{id}/available-asns/

function postAsn-RangesAvailable-Asns ()
{
	...
	Ref: /api/ipam/asn-ranges/{id}/available-asns/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/ipam/asn-ranges/${1}/available-asns" \
