## postPrefixesAvailable-Ips # null
# /api/ipam/prefixes/{id}/available-ips/

function postPrefixesAvailable-Ips ()
{
	...
	Ref: /api/ipam/prefixes/{id}/available-ips/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/ipam/prefixes/${1}/available-ips" \
