## postIp-RangesAvailable-Ips # null
# /api/ipam/ip-ranges/{id}/available-ips/

function postIp-RangesAvailable-Ips ()
{
	...
	Ref: /api/ipam/ip-ranges/{id}/available-ips/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/ipam/ip-ranges/${1}/available-ips" \
