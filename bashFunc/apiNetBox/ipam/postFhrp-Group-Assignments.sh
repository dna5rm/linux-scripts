## postFhrp-Group-Assignments # null
# /api/ipam/fhrp-group-assignments/

function postFhrp-Group-Assignments ()
{
	...
	Ref: /api/ipam/fhrp-group-assignments/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/ipam/fhrp-group-assignments" \
