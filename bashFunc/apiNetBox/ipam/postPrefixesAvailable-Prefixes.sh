## postPrefixesAvailable-Prefixes # null
# /api/ipam/prefixes/{id}/available-prefixes/

function postPrefixesAvailable-Prefixes ()
{
	...
	Ref: /api/ipam/prefixes/{id}/available-prefixes/
	---
	...
	EOF
        ...
        response=$(curl --silent --insecure --location --write-out "%{http_code}" \
          --request POST --url "${netbox_api}/api/ipam/prefixes/${1}/available-prefixes" \
